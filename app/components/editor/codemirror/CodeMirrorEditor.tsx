...
const getCMTranslations = (language: any) => ({
  Find: getTranslation('cm_find', language),
  Replace: getTranslation('cm_replace', language),
  replace: getTranslation('cm_replaceLower', language),
  next: getTranslation('cm_next', language),
  previous: getTranslation('cm_previous', language),
  all: getTranslation('cm_all', language),
  'match case': getTranslation('cm_matchCase', language),
  regexp: getTranslation('cm_regexp', language),
  'by word': getTranslation('cm_byWord', language),
  'replace all': getTranslation('cm_replaceAll', language),
  close: getTranslation('cm_close', language),
});
...
export const CodeMirrorEditor = memo(
  ({
    ...
    useEffect(() => {
      ...
      viewRef.current.dispatch({
        effects: [phrasesCompartment.reconfigure(EditorState.phrases.of(getCMTranslations(language)))],
      });
    }, [language]);
    ...
    useEffect(() => {
      ...
      if (!doc) {
        const state = newEditorState('', theme, settings, onScrollRef, debounceScroll, onSaveRef, [
          ...
          phrasesCompartment.of(EditorState.phrases.of(getCMTranslations(language))),
        ]);
...
function getReadOnlyTooltip(state: EditorState) {
  ...
  let tooltipMessage = getTranslation('cm_readOnlyWarning', language);
  ...
  if (currentDoc?.filePath) {
    ...
    if (locked) {
      tooltipMessage = getTranslation('cm_fileLocked', language);
    }
  }
