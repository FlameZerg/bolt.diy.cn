        ...
        >
          ...
          <div className="p-6 border-b border-bolt-elements-borderColor flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-bolt-elements-textPrimary">{getTranslation('selectBranch', language)}</h3>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center p-8 space-y-4">
                ...
                <p className="text-sm text-bolt-elements-textSecondary">{getTranslation('loadingBranches', language)}</p>
              </div>
            ) : error ? (
              <div className="flex flex-col items-center justify-center p-8 space-y-4">
              ...
              <>
                ...
                {branches.length > 10 && (
                  <div className="p-4 border-b border-bolt-elements-borderColor">
                    <input
                      type="text"
                      placeholder={getTranslation('searchBranchesPlaceholder', language)}
                      value={searchQuery}
                ...
                <div className="flex-1 overflow-y-auto">
                  {filteredBranches.length > 0 ? (
                    ...
                    <div className="flex items-center justify-center p-8">
                      <p className="text-sm text-bolt-elements-textSecondary">
                        {searchQuery ? getTranslation('noBranchesFound', language) : getTranslation('noBranchesAvailable', language)}
                      </p>
                    </div>
                  )}
                </div>
          ...
          {!isLoading && !error && branches.length > 0 && (
            <div className="p-6 border-t border-bolt-elements-borderColor flex items-center justify-between">
              <div className="text-sm text-bolt-elements-textSecondary">
                {selectedBranch && (
                  <>
                    {getTranslation('selectedLabel', language)} <span className="font-medium">{selectedBranch}</span>
                  </>
                )}
              </div>
              <div className="flex items-center gap-3">
                ...
                >
                  {getTranslation('cloneBranch', language)}
                </Button>
              </div>
            </div>
          )}
