...
export const ColorSchemeDialog: React.FC<ColorSchemeDialogProps> = ({ setDesignScheme, designScheme }) => {
  ...
  const getRoleLabel = (key: string) => {
    const labelMap: Record<string, string> = {
      primary: getTranslation('primary', language),
      secondary: getTranslation('secondary', language),
      accent: getTranslation('accent', language),
      background: getTranslation('background', language),
      surface: getTranslation('surface', language),
      text: getTranslation('textPrimary', language),
      textSecondary: getTranslation('textSecondary', language),
      border: getTranslation('border', language),
  ...
  // 获取调色板角色的翻译描述
  const getRoleDescription = (key: string) => {
    const descMap: Record<string, string> = {
      primary: getTranslation('descPrimary', language),
      secondary: getTranslation('descSecondary', language),
      accent: getTranslation('descAccent', language),
      background: getTranslation('descBackground', language),
      surface: getTranslation('descSurface', language),
      text: getTranslation('descText', language),
      textSecondary: getTranslation('descTextSecondary', language),
      border: getTranslation('descBorder', language),
      success: getTranslation('descSuccess', language),
      warning: getTranslation('descWarning', language),
      error: getTranslation('descError', language),
    };
    return descMap[key] || '';
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-bolt-elements-textPrimary flex items-center gap-2">
          ...
          {getTranslation('colorPalette', language)}
        </h3>
        <button
          onClick={handleReset}
          ...
          >
            <div className="relative flex-shrink-0">
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label={`Change ${getRoleLabel(role.key)} color`}
              />
              <input
            ...
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-bolt-elements-textPrimary transition-colors">{getRoleLabel(role.key)}</div>
  ...
  // 获取字体的翻译标签
  const getFontLabel = (key: string) => {
    const fontMap: Record<string, string> = {
      'sans-serif': getTranslation('fontSans', language),
      'serif': getTranslation('fontSerif', language),
      'monospace': getTranslation('fontMono', language),
      'cursive': getTranslation('fontCursive', language),
      'fantasy': getTranslation('fontFantasy', language),
    };
    return fontMap[key] || key;
  };
  ...
  const renderTypographySection = () => (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-bolt-elements-textPrimary flex items-center gap-2">
        <div className="w-2 h-2 rounded-full bg-bolt-elements-item-contentAccent"></div>
        {getTranslation('typography', language)}
      </h3>
  ...
  const getFeatureLabel = (key: string) => {
    const featureMap: Record<string, string> = {
      'rounded': getTranslation('roundedCorners', language),
      'border': getTranslation('subtleBorder', language),
      'gradient': getTranslation('gradientAccent', language),
      'shadow': getTranslation('softShadow', language),
      'frosted-glass': getTranslation('frostedGlass', language),
    };
  ...
  const renderFeaturesSection = () => (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-bolt-elements-textPrimary flex items-center gap-2">
        <div className="w-2 h-2 rounded-full bg-bolt-elements-item-contentAccent"></div>
        {getTranslation('designFeatures', language)}
      </h3>
              ...
              >
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="text-center">
                    <div className="font-semibold">{getFeatureLabel(f.key)}</div>
  ...
  return (
    <div>
      <IconButton title={getTranslation('designPaletteButtonTitle', language)} className="transition-all" onClick={() => setIsDialogOpen(!isDialogOpen)}>
          ...
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              <DialogTitle className="text-2xl font-bold text-bolt-elements-textPrimary">
                {getTranslation('designPaletteFeatures', language)}
              </DialogTitle>
              <DialogDescription className="text-bolt-elements-textSecondary leading-relaxed">
                {getTranslation('designPaletteDesc', language)}
              </DialogDescription>
            </div>
            ...
            <div className="flex gap-1 p-1 bg-bolt-elements-bg-depth-3 rounded-xl">
              {[
                { key: 'colors', label: getTranslation('colors', language), icon: 'i-ph:palette' },
                { key: 'typography', label: getTranslation('typography', language), icon: 'i-ph:text-aa' },
                { key: 'features', label: getTranslation('features', language), icon: 'i-ph:magic-wand' },
              ].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-bolt-elements-textSecondary">
                {Object.keys(palette).length} {getTranslation('colors', language)} • {font.length} {getTranslation('fonts', language)} • {features.length} {getTranslation('features', language)}
              </div>
