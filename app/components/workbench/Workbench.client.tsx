          ...
          {({ open }: { open: boolean }) => (
            <>
              <Popover.Button className="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg bg-bolt-elements-background-depth-2 hover:bg-bolt-elements-background-depth-3 transition-colors text-bolt-elements-item-contentDefault">
                <span>{getTranslation('fileChanges', languageStore.get())}</span>
                {hasChanges && (
                ...
                <Popover.Panel className="absolute right-0 z-20 mt-2 w-80 origin-top-right rounded-xl bg-bolt-elements-background-depth-2 shadow-xl border border-bolt-elements-borderColor">
                  <div className="p-2">
                    <div className="relative mx-2 mb-2">
                      <input
                        type="text"
                        placeholder={getTranslation('searchFiles', languageStore.get())}
                        value={searchQuery}
                      ...
                      {filteredFiles.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center p-4 text-center">
                          ...
                          <p className="text-sm font-medium text-bolt-elements-textPrimary">
                            {searchQuery ? getTranslation('noMatchingFiles', languageStore.get()) : getTranslation('noModifiedFiles', languageStore.get())}
                          </p>
                          <p className="text-xs text-bolt-elements-textTertiary mt-1">
                            {searchQuery ? getTranslation('tryAnotherSearch', languageStore.get()) : getTranslation('changesWillAppear', languageStore.get())}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="border-t border-bolt-elements-borderColor p-2">
                      <button
                        onClick={() => {
                          ...
                          toast(getTranslation('fileListCopied', languageStore.get()), {
                            icon: <div className="i-ph:check-circle text-accent-500" />,
                          });
                        }}
                        className="w-full flex items-center justify-center gap-2 px-3 py-1.5 text-sm rounded-lg bg-bolt-elements-background-depth-1 hover:bg-bolt-elements-background-depth-3 transition-colors text-bolt-elements-textTertiary hover:text-bolt-elements-textPrimary"
                      >
                        {getTranslation('copyFileList', languageStore.get())}
                      </button>
                    </div>
                  )}
                </Popover.Panel>
...
export const Workbench = memo(
  ({
    ...
    const sliderOptions: SliderOptions<WorkbenchViewType> = useMemo(() => ({
      left: {
        value: 'code',
        text: getTranslation('code', language),
      },
      middle: {
        value: 'diff',
        text: getTranslation('diff', language),
      },
      right: {
        value: 'preview',
        text: getTranslation('preview', language),
      },
    }), [language]);
    ...
    const onFileSave = useCallback(() => {
      workbenchStore
        ...
        .catch(() => {
          toast.error(getTranslation('failedToUpdateFileContent', language));
        });
    }, []);
    ...
    const handleSyncFiles = useCallback(async () => {
      ...
      try {
        ...
        toast.success(getTranslation('filesSyncedSuccess', language));
      } catch (error) {
        console.error('Error syncing files:', error);
        toast.error(getTranslation('failedToSyncFiles', language));
      } finally {
        setIsSyncing(false);
      }
    }, []);
    ...
    return (
      chatStarted && (
        ...
        >
          ...
          >
            <div className="absolute inset-0 px-2 lg:px-4 top-[-22px]">
                    ...
                    <div className="flex overflow-y-auto">
                      ...
                      <div className="flex border border-bolt-elements-borderColor rounded-md overflow-hidden ml-1">
                        <DropdownMenu.Root>
                          ...
                          >
                            {isSyncing ? getTranslation('syncing', language) : getTranslation('sync', language)}
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span>{isSyncing ? getTranslation('syncing', language) : getTranslation('syncFiles', language)}</span>
                              </div>
                            </DropdownMenu.Item>
                          </DropdownMenu.Content>
                        </DropdownMenu.Root>
                      ...
                      <div className="flex border border-bolt-elements-borderColor rounded-md overflow-hidden ml-1">
                        ...
                        >
                          ...
                          {getTranslation('toggleTerminal', language)}
                        </button>
                      </div>
                    </div>
                  )}
