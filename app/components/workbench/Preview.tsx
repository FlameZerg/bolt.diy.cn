  ...
  const ResizeHandle = ({ side }: { side: ResizeSide }) => {
    ...
    return (
      <div
        ...
        title={getTranslation('dragToResize', language)}
      >
        <GripIcon />
      </div>
    );
    ...
    if (activePreview?.baseUrl) {
      ...
      if (match) {
        ...
        if (showDeviceFrame && size.hasFrame) {
          ...
          const windowName =
            getTranslation(
              `windowSize_${size.name.replace(/ /g, '').replace(/[^a-zA-Z0-9]/g, '')}` as any,
              language,
            ) || size.name;
          const orientationText = isLandscape
            ? `(${getTranslation('landscape', language)})`
            : `(${getTranslation('portrait', language)})`;
          ...
          const htmlContent = `
            ...
            <head>
              <meta charset="utf-8">
              <title>${windowName} ${getTranslation('preview', language)}</title>
              <style>
                body {
                  margin: 0;
                  padding: 0;
            ...
            <body>
              <div class="device-container">
                <div class="device-name">${windowName} ${orientationText}</div>
    ...
    <div ref={containerRef} className={`w-full h-full flex flex-col relative`}>
      ...
      <div className="bg-bolt-elements-background-depth-2 p-2 flex items-center gap-2">
        ...
        <div className="flex-grow flex items-center gap-1 bg-bolt-elements-preview-addressBar-background border border-bolt-elements-borderColor text-bolt-elements-preview-addressBar-text rounded-full px-1 py-1 text-sm hover:bg-bolt-elements-preview-addressBar-backgroundHover hover:focus-within:bg-bolt-elements-preview-addressBar-backgroundActive focus-within:bg-bolt-elements-preview-addressBar-backgroundActive focus-within-border-bolt-elements-borderColorActive focus-within:text-bolt-elements-preview-addressBar-textActive">
          ...
          />
          <input
            title={getTranslation('urlPath', language)}
            ref={inputRef}
        ...
        <div className="flex items-center gap-2">
          <IconButton
            ...
            title={isDeviceModeOn ? getTranslation('switchToResponsiveMode', language) : getTranslation('switchToDeviceMode', language)}
          />

          {expoUrl && <IconButton icon="i-ph:qr-code" onClick={() => setIsExpoQrModalOpen(true)} title={getTranslation('showQr', language)} />}
          ...
          {isDeviceModeOn && (
            <>
              <IconButton
                ...
                title={isLandscape ? getTranslation('switchToPortrait', language) : getTranslation('switchToLandscape', language)}
              />
              <IconButton
                ...
                title={showDeviceFrameInPreview ? getTranslation('hideDeviceFrame', language) : getTranslation('showDeviceFrame', language)}
              />
            </>
          )}
          <IconButton
            ...
            title={isInspectorMode ? getTranslation('disableInspector', language) : getTranslation('enableInspector', language)}
          />
          <IconButton
          ...
          <div className="flex items-center relative">
            <IconButton
              ...
              title={getTranslation('newWindowOptions', language)}
            />
                ...
                <div className="absolute right-0 top-full mt-2 z-50 min-w-[240px] max-h-[400px] overflow-y-auto bg-white dark:bg-black rounded-xl shadow-2xl border border-[#E5E7EB] dark:border-[rgba(255,255,255,0.1)] overflow-hidden">
                  <div className="p-3 border-b border-[#E5E7EB] dark:border-[rgba(255,255,255,0.1)]">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-[#111827] dark:text-gray-300">{getTranslation('windowOptions', language)}</span>
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        <span>{getTranslation('openInNewTab', language)}</span>
                      ...
                      >
                        <span>{getTranslation('openInNewWindow', language)}</span>
                        <div className="i-ph:browser h-5 w-4" />
                      </button>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-bolt-elements-textTertiary">{getTranslation('showDeviceFrame', language)}</span>
                        <button
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-bolt-elements-textTertiary">{getTranslation('landscapeMode', language)}</span>
                        <button
                    ...
                    >
                      ...
                      <div className="flex-grow flex flex-col">
                        ...
                        <span className="text-xs text-[#6B7280] dark:text-gray-400 group-hover:text-[#6D28D9] dark:group-hover:text-[#6D28D9] transition-colors duration-200">
                          ...
                          {size.hasFrame && showDeviceFrame ? ` ${getTranslation('withFrame', language)}` : ''}
                        </span>
                      </div>
        ...
        >
          {activePreview ? (
              ...
              {isDeviceModeOn && showDeviceFrameInPreview ? (
                ...
                >
                  ...
                  >
                    ...
                    <iframe
                      ref={iframeRef}
                      title={getTranslation('preview', language)}
                      style={{
                        border: 'none',
            ...
            <div className="flex w-full h-full justify-center items-center bg-bolt-elements-background-depth-1 text-bolt-elements-textPrimary">
              {getTranslation('noPreviewAvailable', language)}
            </div>
          )}
