...
function FileContextMenu({
      ...
      for (const item of files) {
        ...
        if (file) {
          try {
            ...
            if (success) {
              toast.success(getTranslation('fileUploadedSuccess', languageStore.get()).replace('{name}', file.name));
            } else {
              toast.error(getTranslation('fileUploadFailed', languageStore.get()).replace('{name}', file.name));
            }
          } catch (error) {
            toast.error(getTranslation('fileUploadError', languageStore.get()).replace('{name}', file.name));
            logger.error(error);
          }
        }
      }
  ...
  const handleCreateFile = async (fileName: string) => {
    ...
    if (success) {
      toast.success(getTranslation('fileCreatedSuccess', languageStore.get()));
    } else {
      toast.error(getTranslation('failedToCreateFile', languageStore.get()));
    }
  ...
  const handleCreateFolder = async (folderName: string) => {
    ...
    if (success) {
      toast.success(getTranslation('folderCreatedSuccess', languageStore.get()));
    } else {
      toast.error(getTranslation('failedToCreateFolder', languageStore.get()));
    }
  ...
  const handleDelete = async () => {
    try {
      const type = isFolder ? getTranslation('folder', languageStore.get()) : getTranslation('file', languageStore.get());
      if (
        !confirm(
          getTranslation('deleteConfirmation', languageStore.get())
            .replace('{type}', type)
      ...
      if (success) {
        toast.success(getTranslation('deleteSuccess', languageStore.get()).replace('{type}', type));
      } else {
        toast.error(getTranslation('deleteFailed', languageStore.get()).replace('{type}', type));
      }
    } catch (error) {
      const type = isFolder ? getTranslation('folder', languageStore.get()) : getTranslation('file', languageStore.get());
      toast.error(getTranslation('deleteError', languageStore.get()).replace('{type}', type));
      logger.error(error);
    }
  };
  ...
  const handleLockFile = () => {
    try {
      ...
      if (success) {
        toast.success(getTranslation('fileLockedSuccess', languageStore.get()));
      } else {
        toast.error(getTranslation('fileLockFailed', languageStore.get()));
      }
    } catch (error) {
      toast.error(getTranslation('fileLockError', languageStore.get()));
      logger.error(error);
    }
  };
  ...
  const handleUnlockFile = () => {
    try {
      ...
      if (success) {
        toast.success(getTranslation('fileUnlockedSuccess', languageStore.get()));
      } else {
        toast.error(getTranslation('fileUnlockFailed', languageStore.get()));
      }
    } catch (error) {
      toast.error(getTranslation('fileUnlockError', languageStore.get()));
      logger.error(error);
    }
  };
  ...
  const handleLockFolder = () => {
    try {
      ...
      if (success) {
        toast.success(getTranslation('folderLockedSuccess', languageStore.get()));
      } else {
        toast.error(getTranslation('folderLockFailed', languageStore.get()));
      }
    } catch (error) {
      toast.error(getTranslation('folderLockError', languageStore.get()));
      logger.error(error);
    }
  };
  ...
  const handleUnlockFolder = () => {
    try {
      ...
      if (success) {
        toast.success(getTranslation('folderUnlockedSuccess', languageStore.get()));
      } else {
        toast.error(getTranslation('folderUnlockFailed', languageStore.get()));
      }
    } catch (error) {
      toast.error(getTranslation('folderUnlockError', languageStore.get()));
      logger.error(error);
    }
  };
  ...
  return (
    <>
            ...
            <ContextMenu.Group className="p-1 border-b-px border-solid border-bolt-elements-borderColor">
              <ContextMenuItem onSelect={() => setIsCreatingFile(true)}>
                <div className="flex items-center gap-2">
                  ...
                  {getTranslation('newFile', languageStore.get())}
                </div>
              </ContextMenuItem>
              <ContextMenuItem onSelect={() => setIsCreatingFolder(true)}>
                <div className="flex items-center gap-2">
                  <div className="i-ph:folder-plus" />
                  {getTranslation('newFolder', languageStore.get())}
                </div>
              </ContextMenuItem>
            </ContextMenu.Group>
            <ContextMenu.Group className="p-1">
              <ContextMenuItem onSelect={onCopyPath}>{getTranslation('copyPath', languageStore.get())}</ContextMenuItem>
              <ContextMenuItem onSelect={onCopyRelativePath}>{getTranslation('copyRelativePath', languageStore.get())}</ContextMenuItem>
            </ContextMenu.Group>
              ...
              {!isFolder ? (
                <>
                  <ContextMenuItem onSelect={handleLockFile}>
                    <div className="flex items-center gap-2">
                      ...
                      {getTranslation('lockFile', languageStore.get())}
                    </div>
                  </ContextMenuItem>
                  <ContextMenuItem onSelect={handleUnlockFile}>
                    <div className="flex items-center gap-2">
                      <div className="i-ph:lock-key-open" />
                      {getTranslation('unlockFile', languageStore.get())}
                    </div>
                  </ContextMenuItem>
                </>
              ) : (
                <>
                  <ContextMenuItem onSelect={handleLockFolder}>
                    <div className="flex items-center gap-2">
                      <div className="i-ph:lock-simple" />
                      {getTranslation('lockFolder', languageStore.get())}
                    </div>
                  </ContextMenuItem>
                  <ContextMenuItem onSelect={handleUnlockFolder}>
                    <div className="flex items-center gap-2">
                      <div className="i-ph:lock-key-open" />
                      {getTranslation('unlockFolder', languageStore.get())}
                    </div>
                  </ContextMenuItem>
                </>
              )}
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-bolt-elements-borderColor">
              <ContextMenuItem onSelect={handleDelete}>
                <div className="flex items-center gap-2 text-red-500">
                  ...
                  {isFolder ? getTranslation('deleteFolder', languageStore.get()) : getTranslation('deleteFile', languageStore.get())}
                </div>
              </ContextMenuItem>
            </ContextMenu.Group>
          </ContextMenu.Content>
      ...
      {isCreatingFile && (
        <InlineInput
          depth={depth}
          placeholder={getTranslation('enterFileName', languageStore.get())}
      ...
      {isCreatingFolder && (
        <InlineInput
          depth={depth}
          placeholder={getTranslation('enterFolderName', languageStore.get())}
        ...
        <div className="flex items-center w-full">
          ...
          {isLocked && (
            <span
              ...
              title={getTranslation('folderIsLocked', languageStore.get())}
            />
          )}
        </div>
      </NodeButton>
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            {locked && (
              <span
                className={classNames('shrink-0', 'i-ph:lock-simple scale-80 text-red-500')}
                title={getTranslation('fileIsLocked', languageStore.get())}
              />
            )}
