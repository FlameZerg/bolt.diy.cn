...
export function LockManager() {
  ...
  const handleUnlockSelected = () => {
    if (selectedItems.size === 0) {
      toast.error(getTranslation('noItemsToUnlock', language));
      return;
    }
    ...
    if (unlockedCount > 0) {
      toast.success(getTranslation('unlockedItems', language).replace('{count}', unlockedCount.toString()));
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-2 py-1 border-b border-bolt-elements-borderColor">
        ...
        >
          ...
          <option value="files">{getTranslation('files', language)}</option>
          <option value="folders">{getTranslation('folders', language)}</option>
        </select>
      </div>
      ...
      <div className="flex items-center justify-between px-2 py-1 text-xs text-bolt-elements-textSecondary">
        <div>
          <Checkbox
            ...
            aria-label={getTranslation('selectAll', language)}
            disabled={filteredAndSortedItems.length === 0} // Disable if no items to select
          />
          <span>{getTranslation('all', language)}</span>
        </div>
        {selectedItems.size > 0 && (
          <button
            ...
            title={getTranslation('unlockAllSelected', language)}
          >
            {getTranslation('unlockAll', language)}
          </button>
        )}
        <div></div>
      </div>
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-bolt-elements-textTertiary text-xs gap-2">
            ...
            <span>{getTranslation('noLockedItems', language)}</span>
          </div>
        ) : (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.success(getTranslation('unlockedPath', language).replace('{path}', item.path.replace('/home/project/', '')));
                  }}
                  title={getTranslation('unlock', language)}
                >
      ...
      <div className="px-2 py-1 border-t border-bolt-elements-borderColor bg-bolt-elements-background-depth-2 text-xs text-bolt-elements-textTertiary flex justify-between items-center">
        <div>
          {getTranslation('lockItemsSelected', language)
            .replace('{count}', filteredAndSortedItems.length.toString())
            .replace('{selected}', selectedItems.size.toString())}
        </div>
      </div>
    </div>
  );
