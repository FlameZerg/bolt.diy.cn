...
const ContentWarning = ({ type, language }: { type: 'binary' | 'error'; language: any }) => (
  <div className="h-full flex items-center justify-center p-4">
    <div className="text-center text-bolt-elements-textTertiary">
      ...
      <p className="font-medium text-bolt-elements-textPrimary">
        {type === 'binary' ? getTranslation('binaryFileDetected', language) : getTranslation('errorProcessingFile', language)}
      </p>
      <p className="text-sm mt-1">
        {type === 'binary'
          ? getTranslation('binaryFileDesc', language)
          : getTranslation('errorDiffDesc', language)}
      </p>
    </div>
  </div>
);
  ...
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-bolt-elements-textTertiary">
        ...
        <p className="font-medium text-bolt-elements-textPrimary">{getTranslation('filesAreIdentical', currentLanguage)}</p>
        <p className="text-sm mt-1">{getTranslation('filesMatchExactly', currentLanguage)}</p>
      </div>
      <div className="mt-4 w-full max-w-2xl bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor overflow-hidden">
        <div className="p-2 text-xs font-bold text-bolt-elements-textTertiary border-b border-bolt-elements-borderColor">
          {getTranslation('currentContent', currentLanguage)}
        </div>
        ...
        <span className="ml-auto shrink-0 flex items-center gap-2">
          {hasChanges ? (
            <>
              ...
              <span className="text-yellow-600 dark:text-yellow-400">{getTranslation('modified', currentLanguage)}</span>
              <span className="text-bolt-elements-textTertiary text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
            <span className="text-green-700 dark:text-green-400">{getTranslation('noChanges', currentLanguage)}</span>
          )}
...
const InlineDiffComparison = memo(({ beforeCode, afterCode, filename, language }: CodeComparisonProps) => {
  ...
  if (isBinary || error) {
    return <ContentWarning type={isBinary ? 'binary' : 'error'} language={currentLanguage} />;
  }
  ...
  if (!highlighter) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-bolt-elements-textTertiary">{getTranslation('loading', currentLanguage)}...</div>
      </div>
    );
  }
      ...
      <div className="w-full h-full flex flex-col">
        ...
        <div className={diffPanelStyles}>
          {hasChanges ? (
            ...
            <NoChangesView beforeCode={beforeCode} language={language} highlighter={highlighter} theme={theme} currentLanguage={currentLanguage} />
          )}
        </div>
      </div>
    </FullscreenOverlay>
