...
export function GitLabDeploymentDialog({ isOpen, onClose, projectName, files }: GitLabDeploymentDialogProps) {
  ...
  const fetchRecentRepos = async (token: string, gitlabUrl = 'https://gitlab.com') => {
    if (!token) {
      ...
      toast.error(getTranslation('deployAuthRequired', language).replace('{provider}', 'GitLab'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!connection?.token || !connection?.user) {
      toast.error(getTranslation('pleaseConnectGitLab', language));
      return;
    }
    ...
    try {
      ...
      if (sanitizedRepoName !== repoName && sanitizedRepoName !== repoName.toLowerCase()) {
        toast.info(getTranslation('deployRepositoryNameSanitized', language).replace('{name}', sanitizedRepoName));
      }
      ...
      if (projectExists && existingProject) {
        ...
        const confirmOverwrite = window.confirm(
          getTranslation('deployExistingRepoWarning', language).replace('{name}', sanitizedRepoName) + visibilityChange,
        );
        ...
        toast.success(getTranslation('repoCreatedSuccess', language));
      }
      ...
      let errorMessage = getTranslation('deployErrorFailedToPush', language).replace('{provider}', 'GitLab');

      if (error instanceof Error) {
        const errorMsg = error.message.toLowerCase();

        if (errorMsg.includes('404') || errorMsg.includes('not found')) {
          errorMessage = getTranslation('deployErrorRepositoryNotFound', language).replace('{provider}', 'GitLab');
        } else if (errorMsg.includes('401') || errorMsg.includes('unauthorized')) {
          errorMessage = getTranslation('deployErrorAuthFailed', language).replace('{provider}', 'GitLab');
        } else if (errorMsg.includes('403') || errorMsg.includes('forbidden')) {
          errorMessage = getTranslation('deployErrorAccessDenied', language).replace('{provider}', 'GitLab');
        } else if (errorMsg.includes('network') || errorMsg.includes('fetch')) {
          errorMessage = getTranslation('deployErrorNetwork', language);
        } else if (errorMsg.includes('timeout')) {
          errorMessage = getTranslation('deployErrorTimeout', language);
        } else if (errorMsg.includes('rate limit')) {
          errorMessage = getTranslation('deployRateLimitExceeded', language)
            .replace('{provider}', 'GitLab')
            .replace('{resetTime}', 'soon');
        } else {
          errorMessage = getTranslation('deployErrorGeneric', language)
            .replace('{provider}', 'GitLab')
            ...
            >
              ...
              >
                <Dialog.Title className="sr-only">
                  {getTranslation('deploySuccessTitle', language).replace('{provider}', 'GitLab')}
                </Dialog.Title>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        >
                          {getTranslation('deploySuccessDesc', language).replace('{provider}', 'GitLab')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                      ...
                      {pushedFiles.length > 100 && (
                        <div className="py-2 text-center text-xs text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary-dark">
                          {getTranslation('moreFiles', language).replace('{count}', (pushedFiles.length - 100).toString())}
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <Dialog.Portal>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[9999]">
            ...
            >
              ...
              >
                <Dialog.Title className="sr-only">
                  {getTranslation('deployConnectionRequired', language).replace('{provider}', 'GitLab')}
                </Dialog.Title>
                <div className="relative text-center space-y-4">
                  ...
                  >
                    {getTranslation('deployConnectionRequiredDesc', language).replace('{provider}', 'GitLab')}
                  </p>
                  <div className="pt-2 flex justify-center gap-3">
                    ...
                    >
                      <div className="i-ph:gitlab-logo w-4 h-4" />
                      {getTranslation('deployConnectAccount', language).replace('{provider}', 'GitLab')}
                    </motion.button>
                  </div>
                </div>
              </Dialog.Content>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <Dialog.Title className="text-lg font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary-dark">
                      {getTranslation('deployTitle', language).replace('{provider}', 'GitLab')}
                    </Dialog.Title>
                    <p
                      id="push-dialog-description"
                      className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary-dark"
                    >
                      {getTranslation('deployDescription', language).replace('{provider}', 'GitLab')}
                    </p>
                  </div>
                  <Dialog.Close asChild>
                    <button
                  ...
                  <div className="space-y-2">
                    ...
                    {recentRepos.length === 0 && !isFetchingRepos ? (
                      <EmptyState
                        ...
                        description={getTranslation('deployNoReposFoundDesc', language).replace('{provider}', 'GitLab')}
                        variant="compact"
                      />
                    ) : (
