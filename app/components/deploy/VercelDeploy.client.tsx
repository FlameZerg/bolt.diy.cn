...
export function useVercelDeploy() {
  ...
  const handleVercelDeploy = async () => {
    if (!vercelConn.user || !vercelConn.token) {
      toast.error(getTranslation('pleaseConnect', language).replace('{provider}', 'Vercel'));
      return false;
    }
    ...
    try {
      ...
      workbenchStore.addArtifact({
        ...
        title: getTranslation('deploymentTitle', language).replace('{provider}', 'Vercel'),
        type: 'standalone',
      });
      ...
      toast.success(getTranslation('deploymentSuccess', language).replace('{provider}', 'Vercel'));

      return true;
    } catch (err) {
      console.error('Vercel deploy error:', err);
      toast.error(err instanceof Error ? err.message : getTranslation('deploymentFailed', language).replace('{provider}', 'Vercel'));
