...
export function useNetlifyDeploy() {
  ...
  const handleNetlifyDeploy = async () => {
    if (!netlifyConn.user || !netlifyConn.token) {
      toast.error(getTranslation('pleaseConnect', language).replace('{provider}', 'Netlify'));
      return false;
    }

    if (!currentChatId) {
      toast.error(getTranslation('noActiveChat', language));
      return false;
    }
    ...
    try {
      ...
      if (!artifact) {
        throw new Error(getTranslation('noActiveProject', language));
      }
      ...
      workbenchStore.addArtifact({
        ...
        title: getTranslation('deploymentTitle', language).replace('{provider}', 'Netlify'),
        type: 'standalone',
      });
      ...
      if (!artifact.runner.buildOutput) {
        ...
        deployArtifact.runner.handleDeployAction('building', 'failed', {
          error: getTranslation('buildFailed', language),
          source: 'netlify',
        });
      ...
      if (!buildPathExists) {
        throw new Error(getTranslation('missingBuildDir', language));
      }
      ...
      toast.success(getTranslation('deploymentSuccess', language).replace('{provider}', 'Netlify'));

      return true;
    } catch (error) {
      console.error('Deploy error:', error);
      toast.error(error instanceof Error ? error.message : getTranslation('deploymentFailed', language).replace('{provider}', 'Netlify'));
