...
export const Menu = () => {
  ...
  const deleteItem = useCallback(
    (event: React.UIEvent, item: ChatHistoryItem) => {
      ...
      deleteChat(item.id)
        .then(() => {
          toast.success(getTranslation('chatDeleted', language), {
        ...
        .catch((error) => {
          ...
          toast.error(getTranslation('deleteChatFailed', language), {
            position: 'bottom-right',
            autoClose: 3000,
          });
  ...
  const deleteSelectedItems = useCallback(
    async (itemsToDeleteIds: string[]) => {
      ...
      if (errors.length === 0) {
        toast.success(getTranslation('bulkDeleteSuccess', language).replace('{0}', deletedCount.toString()));
      } else {
        toast.warning(
          getTranslation('bulkDeletePartial', language)
            .replace('{0}', deletedCount.toString())
            .replace('{1}', itemsToDeleteIds.length.toString())
            .replace('{2}', errors.length.toString()),
          {
            autoClose: 5000,
          },
        );
  ...
  const handleBulkDeleteClick = useCallback(() => {
    if (selectedItems.length === 0) {
      toast.info(getTranslation('selectAtLeastOneChat', language));
      return;
    }
    ...
    if (selectedChats.length === 0) {
      toast.error(getTranslation('couldNotFindSelectedChats', language));
      return;
    }
        ...
        <div className="h-12 flex items-center justify-between px-4 border-b border-gray-100 dark:border-gray-800/50 bg-gray-50/50 dark:bg-gray-900/50 rounded-tr-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-sm text-gray-900 dark:text-white truncate">
              {profile?.username || getTranslation('guestUser', language)}
            </span>
          ...
          <div className="p-4 space-y-3">
            <div className="flex gap-2">
              ...
              >
                ...
                <span className="text-sm font-medium">{getTranslation('newChat', language)}</span>
              </a>
              <button
                ...
                aria-label={selectionMode ? getTranslation('exitSelectionMode', language) : getTranslation('enterSelectionMode', language)}
              >
            ...
            <div className="relative w-full">
              ...
              <input
                ...
                type="search"
                placeholder={getTranslation('searchChats', language)}
                onChange={handleSearchChange}
                aria-label={getTranslation('searchChats', language)}
              />
            </div>
          </div>
          <div className="flex items-center justify-between text-sm px-4 py-2">
            <div className="font-medium text-gray-600 dark:text-gray-400">{getTranslation('history', language)}</div>
            {selectionMode && (
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={selectAll}>
                  {selectedItems.length === filteredList.length ? getTranslation('deselectAll', language) : getTranslation('selectAll', language)}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                ...
                >
                  {getTranslation('deleteSelected', language)}
                </Button>
              </div>
            )}
          </div>
          <div className="flex-1 overflow-auto px-3 pb-3">
            {filteredList.length === 0 && (
              <div className="px-4 text-gray-500 dark:text-gray-400 text-sm">
                {list.length === 0 ? getTranslation('noPreviousConversations', language) : getTranslation('noMatchesFound', language)}
              </div>
            )}
            <DialogRoot open={dialogContent !== null}>
              ...
              <Dialog onBackdrop={closeDialog} onClose={closeDialog}>
                {dialogContent?.type === 'delete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-gray-950">
                      <DialogTitle className="text-gray-900 dark:text-white">{getTranslation('deleteChat', language)}</DialogTitle>
                      <DialogDescription className="mt-2 text-gray-600 dark:text-gray-400">
                        <p>
                          {getTranslation('aboutToDelete', language)}{' '}
                          <span className="font-medium text-gray-900 dark:text-white">
                            {dialogContent.item.description}
                          </span>
                        </p>
                        <p className="mt-2">{getTranslation('deleteChatConfirm', language)}</p>
                      </DialogDescription>
                    </div>
                ...
                {dialogContent?.type === 'bulkDelete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-gray-950">
                      <DialogTitle className="text-gray-900 dark:text-white">{getTranslation('deleteSelectedChats', language)}</DialogTitle>
                      <DialogDescription className="mt-2 text-gray-600 dark:text-gray-400">
                        <p>
                          {getTranslation('aboutToDelete', language)} {dialogContent.items.length}{' '}
                        ...
                        <p className="mt-3">{getTranslation('deleteSelectedChatsConfirm', language)}</p>
                      </DialogDescription>
                    </div>
                    <div className="flex justify-end gap-3 px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
                      <DialogButton type="secondary" onClick={closeDialog}>
