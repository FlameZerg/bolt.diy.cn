...
import { getTranslation, languageStore, type Language } from '~/utils/i18n';
    ...
    <div className="tool-invocation border border-bolt-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          ...
          aria-label={showDetails ? getTranslation('collapseDetails', language) : getTranslation('expandDetails', language)}
        >
          <div className="p-2.5">
          ...
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-bolt-elements-textPrimary font-medium leading-5 text-sm">
              {getTranslation('mcpToolInvocations', language)}{' '}
              {hasToolResults && (
                <span className="w-full w-full text-bolt-elements-textSecondary text-xs mt-0.5">
                  ({getTranslation('toolsUsed', language).replace('{count}', String(toolResults.length))})
                </span>
              )}
            </div>
          </div>
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-bolt-elements-actions-background">
              <ToolResultsList toolInvocations={toolResults} toolCallAnnotations={toolCallAnnotations} theme={theme} language={language} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
const ToolResultsList = memo(({ toolInvocations, toolCallAnnotations, theme, language }: ToolResultsListProps) => {
  return (
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                ...
                <div className="text-bolt-elements-textSecondary text-xs">{getTranslation('server', language)}:</div>
              ...
              <div className="ml-6 mb-2">
                <div className="text-bolt-elements-textSecondary text-xs mb-1">
                  {getTranslation('tool', language)}: <span className="text-bolt-elements-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-bolt-elements-textSecondary text-xs mb-1">
                  {getTranslation('description', language)}:{' '}
                  <span className="text-bolt-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-bolt-elements-textSecondary text-xs mb-1">{getTranslation('parameters', language)}:</div>
                ...
                <div className="text-bolt-elements-textSecondary text-xs mt-3 mb-1">{getTranslation('result', language)}:</div>
                <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
...
const ToolCallsList = memo(({ toolInvocations, toolCallAnnotations, addToolResult, language }: ToolCallsListProps) => {
              ...
              <div className="bg-bolt-elements-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  ...
                  <div className="flex items-center justify-end gap-2 ml-auto">
                    ...
                    >
                      {getTranslation('cancelTool', language)} <span className="opacity-70 text-xs ml-1">{isMac ? '⌘⌫' : 'Ctrl+Backspace'}</span>
                    </button>
                    <button
                      className={classNames(
                    ...
                    >
                      {getTranslation('runTool', language)} <span className="opacity-70 text-xs ml-1">{isMac ? '⌘↵' : 'Ctrl+Enter'}</span>
                    </button>
                  </div>
                </div>
              </div>
