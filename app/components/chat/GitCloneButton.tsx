    ...
    try {
      ...
      if (importChat) {
        ...
        for (const filePath of filePaths) {
          ...
          try {
            ...
            if (fileSize > MAX_FILE_SIZE) {
              skippedFiles.push(
                getTranslation('fileTooLarge', language)
                  .replace('{path}', filePath)
                  .replace('{size}', String(Math.round(fileSize / 1024))),
              );
              continue;
            }
            ...
            if (totalSize + fileSize > MAX_TOTAL_SIZE) {
              skippedFiles.push(getTranslation('fileExceedsLimit', language).replace('{path}', filePath));
              continue;
            }
            ...
            skippedFiles.push(
              getTranslation('fileError', language).replace('{path}', filePath).replace('{message}', e.message),
            );
          }
        }
        ...
        const filesMessage: Message = {
          role: 'assistant',
          content: `${getTranslation('cloningRepoMessage', language)
            .replace('{repoUrl}', repoUrl)
            .replace('{workdir}', workdir)}
${skippedFiles.length > 0
              ? `\n${getTranslation('skippedFiles', language)} (${skippedFiles.length}):
${skippedFiles.map((f) => `- ${f}`).join('\n')}`
              : ''
            }

<boltArtifact id="imported-files" title="${getTranslation('gitClonedFiles', language)}" type="bundled">
${fileContents
              .map(
                (file) =>
  ...
  return (
    <>
      <Button
        ...
        title={getTranslation('cloneARepo', language)}
        variant="default"
        size="lg"
      ...
      >
        {getTranslation('cloneARepo', language)}
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                  {getTranslation('chooseRepoProvider', language)}
                </h3>
                <button
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
                        {getTranslation('cloneFromGitHub', language)}
                      </div>
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
                        {getTranslation('cloneFromGitLab', language)}
                      </div>
                    </div>
                  </div>
                </button>
          ...
          <div className="bg-white dark:bg-gray-950 rounded-xl shadow-xl border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor w-full max-w-4xl max-h-[90vh] overflow-hidden">
            <div className="p-6 border-b border-bolt-elements-borderColor dark:border-bolt-elements-borderColor flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                    {getTranslation('importGitHubRepo', language)}
                  </h3>
                  <p className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
                    {getTranslation('cloneGitHubDesc', language)}
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="bg-white dark:bg-gray-950 rounded-xl shadow-xl border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor w-full max-w-4xl max-h-[90vh] overflow-hidden">
            <div className="p-6 border-b border-bolt-elements-borderColor dark:border-bolt-elements-borderColor flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                    {getTranslation('importGitLabRepo', language)}
                  </h3>
                  <p className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
                    {getTranslation('cloneGitLabDesc', language)}
                  </p>
                </div>
              </div>
              <button
      ...
      {loading && <LoadingOverlay message={getTranslation('cloningRepoWait', language)} />}
    </>
  );
}
