    ...
    return (
      <div className="mb-2 p-4 rounded-lg border border-bolt-elements-borderColor bg-bolt-elements-prompt-background text-bolt-elements-textPrimary">
        <p className="text-center">
          {getTranslation('noProvidersEnabled', language)}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-2 flex-col sm:flex-row">
      ...
      <div className="relative flex w-full" onKeyDown={handleProviderKeyDown} ref={providerDropdownRef}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="truncate">{provider?.name || getTranslation('selectProvider', language)}</div>
            <div
              className={classNames(
            ...
            <div className="px-2 pb-2">
              <div className="relative">
                <input
                  ...
                  placeholder={getTranslation('searchProviders', language)}
                  className={classNames(
                ...
                <div className="px-3 py-3 text-sm">
                  <div className="text-bolt-elements-textTertiary mb-1">
                    {debouncedProviderSearchQuery
                      ? getTranslation('noProvidersMatch', language).replace('{query}', debouncedProviderSearchQuery)
                      : getTranslation('noProvidersFound', language)}
                  </div>
                  {debouncedProviderSearchQuery && (
                    <div className="text-xs text-bolt-elements-textTertiary">
                      {getTranslation('providerSearchHint', language)}
                    </div>
                  )}
                </div>
              ) : (
      ...
      <div className="relative flex w-full min-w-[70%]" onKeyDown={handleModelKeyDown} ref={modelDropdownRef}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="truncate">{modelList.find((m) => m.name === model)?.label || getTranslation('selectModel', language)}</div>
            <div
              className={classNames(
                'i-ph:caret-down w-4 h-4 text-bolt-elements-textSecondary opacity-75',
            ...
            <div className="px-2 pb-2 space-y-2">
              ...
              {provider?.name === 'OpenRouter' && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {getTranslation('freeModelsOnly', language)}
                  </button>
                  {showFreeModelsOnly && (
              ...
              {debouncedModelSearchQuery && filteredModels.length > 0 && (
                <div className="text-xs text-bolt-elements-textTertiary px-1">
                  {filteredModels.length} {getTranslation('modelsFound', language).replace('{count}', filteredModels.length.toString()).replace('{plural}', filteredModels.length !== 1 ? 's' : '')}
                  {filteredModels.length > 5 && ` ${getTranslation('showingBestMatches', language)}`}
                </div>
              )}
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={getTranslation('searchModels', language)}
                  className={classNames(
                    'w-full pl-8 pr-8 py-1.5 rounded-md text-sm',
                    'bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor',
                    'text-bolt-elements-textPrimary placeholder:text-bolt-elements-textTertiary',
              ...
              {modelLoading === 'all' || modelLoading === provider?.name ? (
                <div className="px-3 py-3 text-sm">
                  <div className="flex items-center gap-2 text-bolt-elements-textTertiary">
                    ...
                    {getTranslation('loadingModels', language)}
                  </div>
                </div>
                ...
                <div className="px-3 py-3 text-sm">
                  <div className="text-bolt-elements-textTertiary mb-1">
                    {debouncedModelSearchQuery
                      ? getTranslation('noModelsMatch', language).replace('{query}', debouncedModelSearchQuery) + (showFreeModelsOnly ? ` (${getTranslation('freeModelsOnly', language)})` : '')
                      : showFreeModelsOnly
                        ? getTranslation('noFreeModelsAvailable', language)
                        : getTranslation('noModelsAvailable', language)}
                  </div>
                  {debouncedModelSearchQuery && (
                    <div className="text-xs text-bolt-elements-textTertiary">
                      {getTranslation('modelSearchHint', language)}
                    </div>
                  )}
                  {showFreeModelsOnly && !debouncedModelSearchQuery && (
                    <div className="text-xs text-bolt-elements-textTertiary">
                      {getTranslation('disableFreeFilter', language)}
                    </div>
                  )}
                </div>
              ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs text-bolt-elements-textTertiary">
                            {formatContextSize(modelOption.maxTokenAllowed)} {getTranslation('tokens', language)}
                          </span>
                          {debouncedModelSearchQuery && (modelOption as any).searchScore > 70 && (
                            <span className="text-xs text-green-500 font-medium">
                              {getTranslation('percentMatch', language).replace('{percent}', (modelOption as any).searchScore.toFixed(0))}
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 ml-2">
                        {isModelLikelyFree(modelOption, provider?.name) && (
                          <span className="i-ph:gift text-xs text-purple-400" title={getTranslation('freeModel', language)} />
                        )}
                        {model === modelOption.name && (
                          <span className="i-ph:check text-xs text-green-500" title={getTranslation('selectedModel', language)} />
                        )}
                      </div>
                    </div>
                  </div>
