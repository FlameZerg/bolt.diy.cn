...
export function McpTools() {
  ...
  const checkServerAvailability = async () => {
    ...
    try {
      ...
      setError(getTranslation('failedToCheckServerAvailability', language).replace('{error}', e instanceof Error ? e.message : String(e)));
    } finally {
    ...
    <div className="relative">
      <div className="flex">
        <IconButton
          ...
          title={getTranslation('mcpToolsAvailable', language)}
        ...
        {isDialogOpen && (
          <Dialog className="max-w-4xl w-full p-6">
            <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
              <DialogTitle>
                <div className="i-bolt:mcp text-xl"></div>
                {getTranslation('mcpTools', language)}
              </DialogTitle>
                ...
                <div>
                  ...
                  {serverEntries.length > 0 ? (
                    ...
                    <div className="py-4 text-center text-bolt-elements-textSecondary">
                      <p>{getTranslation('noMcpServersConfigured', language)}</p>
                      <p className="text-xs mt-1">{getTranslation('configureMcpServers', language)}</p>
                    </div>
                  )}
                </div>
