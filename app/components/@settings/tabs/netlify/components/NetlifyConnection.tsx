...
export default function NetlifyConnection() {
        ...
        try {
          ...
          if (!siteResponse.ok) {
            ...
            if (siteResponse.status === 404) {
              toast.error(getTranslation('siteNotFoundFree', languageStore.get()));
              return;
            }
          ...
          if (siteData.build_settings && siteData.build_settings.repo_url) {
            ...
            if (buildResponse.ok) {
              toast.success(getTranslation('buildTriggered', languageStore.get()));
              return;
            } else if (buildResponse.status === 422) {
              // Often indicates free account limitation
              toast.warning(getTranslation('buildTriggerFailed', languageStore.get()));
              return;
            }
          }
          ...
          if (!cacheResponse.ok) {
            if (cacheResponse.status === 404) {
              if (isFreeAccount) {
                toast.warning(getTranslation('cachePurgeNotAvailable', languageStore.get()));
              } else {
                toast.error(getTranslation('cachePurgeNotFound', languageStore.get()));
              }
          ...
          toast.success(getTranslation('siteCacheCleared', languageStore.get()));
        } catch (err: unknown) {
      ...
      action: async (siteId: string) => {
        try {
          ...
          if (envResponse.ok) {
            ...
            if (isFreeAccount) {
              toast.info(getTranslation('envVarLimited', languageStore.get()));
            } else {
              toast.info(getTranslation('noEnvVars', languageStore.get()));
            }
          } else {
      ...
      action: async (siteId: string) => {
        try {
          ...
          if (functionsResponse.ok) {
            ...
            if (isFreeAccount) {
              toast.info(getTranslation('functionsLimited', languageStore.get()));
            } else {
              toast.info(getTranslation('noFunctions', languageStore.get()));
            }
          } else {
      ...
      action: async (siteId: string) => {
        try {
          ...
          if (analyticsResponse.ok) {
            ...
            toast.success(getTranslation('analyticsLoaded', languageStore.get()));
          } else if (analyticsResponse.status === 404) {
            if (isFreeAccount) {
              toast.info(getTranslation('analyticsLimited', languageStore.get()));
            }
    ...
    },
    {
      ...
      action: async (siteId: string) => {
        try {
          ...
          toast.success(getTranslation('siteDeleted', languageStore.get()));
  ...
  const handleConnect = async () => {
    if (!tokenInput) {
      toast.error(getTranslation('enterNetlifyToken', languageStore.get()));
      return;
    }
    ...
    try {
      ...
      toast.success(getTranslation('netlifyConnectSuccess', languageStore.get()));
  ...
  const handleDisconnect = () => {
    ...
    toast.success(getTranslation('netlifyDisconnectSuccess', languageStore.get()));
  };
  ...
  const fetchNetlifyStats = async (token: string) => {
    ...
    try {
      ...
      toast.success(getTranslation('statsUpdated', languageStore.get()));
    } catch (error) {
      ...
      <div className="p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-medium text-bolt-elements-textPrimary">{getTranslation('netlifyConnection', language)}</h2>
          </div>
        </div>
        ...
        {!connection.user ? (
          <div className="mt-4">
            ...
            <div className="mt-2 text-sm text-bolt-elements-textSecondary">
              ...
              >
                {getTranslation('getNetlifyToken', language)}
