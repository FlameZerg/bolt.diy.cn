...
export default function NetlifyTab() {
  ...
  const testConnection = async () => {
    if (!connection.token) {
      setConnectionTest({
        status: 'error',
        message: getTranslation('noTokenProvided', language),
        timestamp: Date.now(),
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setConnectionTest({
          status: 'success',
          message: getTranslation('connectedSuccessfully', language),
          timestamp: Date.now(),
        });
      } else {
        setConnectionTest({
          status: 'error',
          message: getTranslation('connectionFailed', language).replace('{error}', `${response.status} ${response.statusText}`),
          timestamp: Date.now(),
        });
      }
    } catch (error) {
  ...
  const siteActions: SiteAction[] = [
    {
      name: getTranslation('clearCache', language),
        ...
        try {
          ...
          if (!siteResponse.ok) {
            ...
            if (siteResponse.status === 404) {
              toast.error(getTranslation('siteNotFound', language));
              return;
            }

            throw new Error(`${getTranslation('failedToGetSiteDetails', language)}: ${errorText}`);
          }
          ...
          if (siteData.build_settings && siteData.build_settings.repo_url) {
            ...
            if (buildResponse.ok) {
              toast.success(getTranslation('buildTriggeredCacheClear', language));
              return;
            } else if (buildResponse.status === 422) {
              // Often indicates free account limitation
              toast.warning(getTranslation('buildTriggerFailed', language));
              return;
            }
          }
          ...
          if (!cacheResponse.ok) {
            if (cacheResponse.status === 404) {
              if (isFreeAccount) {
                toast.warning(getTranslation('cachePurgeNotAvailable', language));
              } else {
                toast.error(getTranslation('cachePurgeEndpointNotFound', language));
              }
            ...
            throw new Error(`${getTranslation('failedToClearCache', language)}: ${errorText}`);
          }

          toast.success(getTranslation('siteCacheCleared', language));
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToClearCache', language).replace('{error}', error));
        } finally {
    ...
    {
      name: getTranslation('manageEnv', language),
      icon: 'i-ph:gear',
      action: async (siteId: string) => {
        try {
          setIsActionLoading(true);
          ...
          if (!siteResponse.ok) {
            throw new Error(getTranslation('failedToGetSiteDetails', language));
          }
          ...
          if (envResponse.ok) {
            ...
            toast.success(getTranslation('envVarsLoaded', language).replace('{count}', envVars.length.toString()));
          } else if (envResponse.status === 404) {
            if (isFreeAccount) {
              toast.info(getTranslation('envVarsLimited', language));
            } else {
              toast.info(getTranslation('noEnvVarsConfigured', language));
            }
          } else {
            const errorText = await envResponse.text();
            toast.error(getTranslation('failedToLoadEnvVars', language).replace('{error}', errorText));
          }
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToLoadEnvVars', language).replace('{error}', error));
        } finally {
          setIsActionLoading(false);
        }
      },
    ...
    {
      name: getTranslation('triggerBuild', language),
      ...
      action: async (siteId: string) => {
        try {
          ...
          if (!buildResponse.ok) {
            throw new Error(getTranslation('failedToTriggerBuild', language));
          }

          const buildData = (await buildResponse.json()) as any;
          toast.success(getTranslation('buildTriggeredSuccess', language).replace('{id}', buildData.id));
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToTriggerBuild', language).replace('{error}', error));
        } finally {
          setIsActionLoading(false);
        }
      },
      ...
      action: async (siteId: string) => {
        try {
          ...
          if (functionsResponse.ok) {
            ...
            toast.success(getTranslation('siteHasFunctions', language).replace('{count}', functions.length.toString()));
          } else if (functionsResponse.status === 404) {
            if (isFreeAccount) {
              toast.info(getTranslation('functionsLimited', language));
            } else {
              toast.info(getTranslation('noFunctions', language));
            }
          } else {
            const errorText = await functionsResponse.text();
            toast.error(getTranslation('failedToLoadFunctions', language).replace('{error}', errorText));
          }
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToLoadFunctions', language).replace('{error}', error));
        } finally {
          setIsActionLoading(false);
        }
      },
    ...
    {
      name: getTranslation('siteAnalytics', language),
      icon: 'i-ph:chart-bar',
      action: async (siteId: string) => {
        try {
          setIsActionLoading(true);
          ...
          if (analyticsResponse.ok) {
            ...
            toast.success(getTranslation('analyticsLoaded', language));
          } else if (analyticsResponse.status === 404) {
            if (isFreeAccount) {
              toast.info(getTranslation('analyticsLimited', language));
            }
            ...
            if (isFreeAccount) {
              toast.info(
                getTranslation('analyticsUnavailable', language).replace('{info}', `${siteData.name} (${siteData.state || getTranslation('unknown', language)})`),
              );
            } else {
              toast.error(getTranslation('failedToLoadAnalytics', language).replace('{error}', errorText));
            }
          }
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToLoadAnalytics', language).replace('{error}', error));
        } finally {
          setIsActionLoading(false);
        }
      },
    ...
    {
      name: getTranslation('deleteSite', language),
      icon: 'i-ph:trash',
      action: async (siteId: string) => {
        try {
          ...
          if (!response.ok) {
            throw new Error(getTranslation('failedToDeleteSite', language));
          }

          toast.success(getTranslation('siteDeleted', language));
          fetchNetlifyStats(connection.token);
        } catch (err: unknown) {
          const error = err instanceof Error ? err.message : 'Unknown error';
          toast.error(getTranslation('failedToDeleteSite', language).replace('{error}', error));
        }
      },
  ...
  const handleDeploy = async (siteId: string, deployId: string, action: 'lock' | 'unlock' | 'publish') => {
    try {
      ...
      if (!response.ok) {
        throw new Error(getTranslation('failedToUpdateDeploy', language).replace('{action}', action));
      }

      toast.success(getTranslation('deployUpdated', language).replace('{action}', action));
      fetchNetlifyStats(connection.token);
    } catch (err: unknown) {
      const error = err instanceof Error ? err.message : 'Unknown error';
      toast.error(getTranslation('failedToUpdateDeploy', language).replace('{action}', action).replace('{error}', error));
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  const handleConnect = async () => {
    if (!tokenInput) {
      toast.error(getTranslation('pleaseEnterToken', language));
      return;
    }
    ...
    try {
      ...
      toast.success(getTranslation('connectedToNetlify', language));

      // Fetch stats after successful connection
      fetchNetlifyStats(tokenInput);
    } catch (error) {
      console.error('Error connecting to Netlify:', error);
      toast.error(getTranslation('failedToConnectNetlify', language).replace('{error}', error instanceof Error ? error.message : getTranslation('unknownError', language)));
    } finally {
  ...
  const handleDisconnect = () => {
    ...
    toast.success(getTranslation('disconnectedFromNetlify', language));
  };
  ...
  const fetchNetlifyStats = async (token: string) => {
    ...
    try {
      ...
      if (!sitesResponse.ok) {
        throw new Error(`${getTranslation('failedToFetchSites', language) || 'Failed to fetch sites'}: ${sitesResponse.statusText}`);
      }
      ...
      toast.success(getTranslation('netlifyStatsUpdated', language));
    } catch (error) {
      console.error('Error fetching Netlify stats:', error);
      toast.error(getTranslation('failedToFetchNetlifyStats', language).replace('{error}', error instanceof Error ? error.message : getTranslation('unknownError', language)));
    } finally {
            ...
            <div className="flex items-center justify-between p-4 rounded-lg bg-bolt-elements-background dark:bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor hover:border-bolt-elements-borderColorActive/70 dark:hover:border-bolt-elements-borderColorActive/70 transition-all duration-200 cursor-pointer">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                  {getTranslation('netlifyStats', language)}
                </span>
              </div>
              <div
          ...
          <CollapsibleContent className="overflow-hidden">
            <div className="space-y-4 mt-4">
              ...
              <div className="mb-6 p-4 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{getTranslation('netlifyOverview', language)}</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {connection.stats.totalSites}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('totalSites', language)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {connection.stats.totalDeploys || deploymentCount}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('totalDeployments', language)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {connection.stats.totalBuilds || 0}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('totalBuilds', language)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {sites.filter((site) => site.published_deploy?.state === 'ready').length}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('liveSites', language)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6 space-y-4">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary">{getTranslation('deploymentAnalytics', language)}</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg border border-bolt-elements-borderColor">
                    <h6 className="text-xs font-medium text-bolt-elements-textPrimary flex items-center gap-2 mb-2">
                      ...
                      {getTranslation('successRate', language)}
                    </h6>
                    <div className="space-y-1">
                      {(() => {
                        ...
                        return [
                          { label: getTranslation('successRate', language), value: `${successRate}%` },
                          { label: getTranslation('success', language), value: successfulDeploys },
                          { label: getTranslation('error', language), value: failedDeploys },
                        ];
                      })().map((item, idx) => (
                  ...
                  <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg border border-bolt-elements-borderColor">
                    ...
                    <div className="space-y-1">
                      {(() => {
                        ...
                        return [
                          { label: getTranslation('last24Hours', language) || 'Last 24 hours', value: last24Hours },
                          { label: getTranslation('last7Days', language) || 'Last 7 days', value: last7Days },
                          { label: getTranslation('activeSites', language) || 'Active sites', value: activeSites },
                        ];
                      })().map((item, idx) => (
                        <div key={idx} className="flex justify-between text-xs">
                          <span className="text-bolt-elements-textSecondary">{item.label}:</span>
              ...
              {/* Site Health Metrics */}
              <div className="mb-6">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('siteHealthOverview', language)}</h4>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {(() => {
                    ...
                    return [
                      {
                        label: getTranslation('healthy', language),
                        value: healthySites,
                        icon: 'i-ph:heart',
                      ...
                      {
                        label: getTranslation('sslEnabled', language) || 'SSL Enabled',
                        value: sslEnabled,
                        icon: 'i-ph:lock',
                        color: 'text-blue-500',
                      ...
                      {
                        label: getTranslation('customDomain', language) || 'Custom Domain',
                        value: customDomain,
                        icon: 'i-ph:globe',
                      ...
                      {
                        label: getTranslation('building', language) || 'Building',
                        value: buildingSites,
                        icon: 'i-ph:gear',
                      ...
                      {
                        label: getTranslation('needsAttention', language) || 'Needs Attention',
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                >
                  ...
                  <span>{connection.stats.totalSites} {getTranslation('sites', language)}</span>
                </Badge>
                <Badge
                  variant="outline"
                  className="flex items-center gap-1 text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary"
                >
                  ...
                  <span>{deploymentCount} {getTranslation('deployments', language)}</span>
                </Badge>
                <Badge
                  variant="outline"
                  className="flex items-center gap-1 text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary"
                >
                  ...
                  <span>{connection.stats.totalBuilds || 0} {getTranslation('builds', language)}</span>
                </Badge>
                {lastUpdated && (
                  <Badge
                    variant="outline"
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        <h4 className="text-sm font-medium flex items-center gap-2 text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                          ...
                          {getTranslation('yourSites', language)} ({sites.length})
                        </h4>
                        {sites.length > 8 && (
                          <button
                          ...
                          >
                            {isSitesExpanded ? getTranslation('showLess', language) : `${getTranslation('showAll', language)} ${sites.length}`}
                          </button>
                        )}
                      </div>
                      <Button
                                    ...
                                    <Button
                                      ...
                                      onClick={async (e) => {
                                        ...
                                        if (action.requiresConfirmation) {
                                          if (!confirm(getTranslation('confirmAction', language).replace('{action}', action.name.toLowerCase()))) {
                                            return;
                                          }
                                        }
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {deploys.length > 10 && (
                            <button
                              ...
                              className="text-xs text-bolt-elements-textSecondary hover:text-bolt-elements-textPrimary transition-colors"
                            >
                              {isDeploysExpanded ? getTranslation('showLess', language) : `${getTranslation('showAll', language)} ${deploys.length}`}
                            </button>
                          )}
                        </div>
                      </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <Badge variant={build.done ? 'default' : 'outline'} className="flex items-center gap-1">
                                  ...
                                  <span className="text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
                                    {build.done ? getTranslation('completed', language) : getTranslation('building', language)}
                                  </span>
                                </Badge>
                              </div>
                              <span className="text-xs text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
            {getTranslation('netlifyIntegration', language)}
          </h2>
        </div>
        <div className="flex items-center gap-2">
          {connection.user && (
      ...
      <p className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
        {getTranslation('netlifyDescription', language)}
      </p>
            ...
            <div className="space-y-4">
              <div className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 dark:bg-bolt-elements-background-depth-1 p-3 rounded-lg mb-4">
                <p className="flex items-center gap-1 mb-1">
                  ...
                  <span className="font-medium">{getTranslation('tip', language)}:</span> {getTranslation('netlifyEnvTip', language)}
                </p>
              </div>

              <div>
                <label className="block text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary mb-2">
                  {getTranslation('apiToken', language)}
                </label>
                <input
                  type="password"
                  value={tokenInput}
                  onChange={(e) => setTokenInput(e.target.value)}
                  placeholder={getTranslation('enterNetlifyToken', language)}
                  className={classNames(
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-bolt-elements-textSecondary flex items-center gap-1">
                  <div className="i-ph:check-circle w-4 h-4 text-green-500" />
                  {getTranslation('connectedToNetlify', language)}
                </span>
              </div>
              {renderStats()}
            </div>
