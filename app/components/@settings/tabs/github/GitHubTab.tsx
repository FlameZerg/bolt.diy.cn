...
export default function GitHubTab() {
  ...
  const handleTestConnection = async () => {
    if (!connection?.user) {
      setConnectionTest({
        status: 'error',
        message: getTranslation('noConnectionEstablished', language),
        timestamp: Date.now(),
      });
      return;
    }
    ...
    try {
      ...
      if (isValid) {
        setConnectionTest({
          status: 'success',
          message: getTranslation('connectedSuccessfully', language).replace('{user}', connection.user.login),
          timestamp: Date.now(),
        });
      } else {
        setConnectionTest({
          status: 'error',
          message: getTranslation('connectionTestFailed', language),
          timestamp: Date.now(),
        });
      }
    } catch (error) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <GithubLogo />
          <h2 className="text-lg font-medium text-bolt-elements-textPrimary">{getTranslation('githubIntegration', language)}</h2>
        </div>
        <LoadingState message={getTranslation('checkingGitHubConnection', language)} />
      </div>
    );
  }
  ...
  if (error && !connection) {
    return (
      <div className="space-y-6">
        ...
        <ErrorState
          title={getTranslation('connectionError', language)}
          message={error}
          onRetry={() => window.location.reload()}
          retryLabel={getTranslation('reloadPage', language)}
        />
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-bolt-elements-textSecondary">
          {getTranslation('githubDescription', language)}
        </p>
  ...
  return (
    <GitHubErrorBoundary>
      <div className="space-y-6">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
              {getTranslation('githubIntegration', language)}
            </h2>
          </div>
          <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between p-4 rounded-lg bg-bolt-elements-background dark:bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor hover:border-bolt-elements-borderColorActive/70 dark:hover:border-bolt-elements-borderColorActive/70 transition-all duration-200">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-bolt-elements-textPrimary">
                      {getTranslation('allRepositories', language)} ({stats.repos.length})
                    </span>
                  </div>
                  <ChevronDown
                ...
                <div className="mt-4 space-y-4">
                  ...
                  {stats.repos.length > 12 && !isReposExpanded && (
                    <div className="text-center">
                      ...
                      >
                        {getTranslation('showMoreRepositories', language).replace('{count}', (stats.repos.length - 12).toString())}
                      </Button>
                    </div>
                  )}
                </div>
        ...
        {statsError && !stats && (
          <ErrorState
            title={getTranslation('failedToLoadStats', language)}
            message={statsError}
            onRetry={() => window.location.reload()}
            retryLabel={getTranslation('retry', language)}
          />
        )}
        ...
        {isStatsLoading && !stats && (
          <GitHubProgressiveLoader
            isLoading={isStatsLoading}
            loadingMessage={getTranslation('loadingGitHubStats', language)}
            showProgress={true}
            progressSteps={[
              { key: 'user', label: getTranslation('fetchingUserInfo', language), completed: !!connection?.user, loading: !connection?.user },
              { key: 'repos', label: getTranslation('loadingRepositories', language), completed: false, loading: true },
              { key: 'stats', label: getTranslation('calculatingStats', language), completed: false },
              { key: 'cache', label: getTranslation('updatingCache', language), completed: false },
            ]}
          >
            <div />
          </GitHubProgressiveLoader>
