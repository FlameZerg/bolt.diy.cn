          ...
          >
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-bolt-elements-textPrimary">{getTranslation('connectToGitHub', language)}</h2>
                <button
                  onClick={handleClose}
              ...
              <div className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 p-3 rounded-lg">
                <p className="flex items-center gap-1 mb-1">
                  ...
                  <span className="font-medium">{getTranslation('tip', language)}:</span> {getTranslation('githubTokenTip', language)}
                </p>
                <p>{getTranslation('requiredScopes', language)}: {getTranslation('requiredScopesDetail', language)}</p>
              </div>

              <form onSubmit={handleConnect} className="space-y-4">
                <div>
                  <label className="block text-sm text-bolt-elements-textSecondary mb-2">{getTranslation('tokenType', language)}</label>
                  <select
                    value={tokenType}
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={isConnecting}
                    placeholder={tokenType === 'classic' ? getTranslation('enterPersonalAccessToken', language) : getTranslation('enterFineGrainedToken', language)}
                    className={classNames(
                      'w-full px-3 py-2 rounded-lg text-sm',
                      'bg-bolt-elements-background-depth-1',
                      'border border-bolt-elements-borderColor',
