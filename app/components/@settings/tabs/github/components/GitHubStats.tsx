          ...
          <div className="flex items-center gap-2">
            {isLoading ? (
              <>
                ...
                <span className="text-bolt-elements-textSecondary">{getTranslation('loadingGitHubStats', language)}</span>
              </>
            ) : (
              <span className="text-bolt-elements-textSecondary">{getTranslation('noStatsAvailable', language)}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <Collapsible open={isExpanded} onOpenChange={onToggleExpanded}>
          ...
          <div className="flex items-center justify-between p-4 rounded-lg bg-bolt-elements-background dark:bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor hover:border-bolt-elements-borderColorActive/70 dark:hover:border-bolt-elements-borderColorActive/70 transition-all duration-200">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-bolt-elements-textPrimary">
                {getTranslation('githubStats', language)}
                {isStale && <span className="text-bolt-elements-textTertiary ml-1">{getTranslation('stale', language)}</span>}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
              ...
              >
                {isRefreshing ? (
                  <>
                    ...
                    {getTranslation('refreshing', language)}
                  </>
                ) : (
                  <>
        ...
        <CollapsibleContent className="overflow-hidden">
          <div className="space-y-4 mt-4">
            {/* Languages Section */}
            <div className="mb-6">
              <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{getTranslation('topLanguages', language)}</h4>
              {stats.mostUsedLanguages && stats.mostUsedLanguages.length > 0 ? (
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-bolt-elements-textSecondary">
                    {getTranslation('basedOnCodebaseSize', language)}
                  </div>
                </div>
              ) : (
                <div className="flex flex-wrap gap-2">
            ...
            <div className="mb-6 p-4 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
              <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{getTranslation('githubOverview', language)}</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {(stats.publicRepos || 0) + (stats.privateRepos || 0)}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('totalRepositories', language)}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">{stats.totalBranches || 0}</div>
                  <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('totalBranches', language)}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {stats.organizations?.length || 0}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('organizations', language)}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {Object.keys(stats.languages).length}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('languagesUsed', language)}</div>
                </div>
              </div>
            </div>
            ...
            {/* Activity Summary */}
            <div className="mb-6">
              <h5 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('activitySummary', language)}</h5>
              <div className="grid grid-cols-4 gap-4">
                {[
                  {
                    label: getTranslation('totalBranches', language),
                    value: stats.totalBranches || 0,
                    icon: 'i-ph:git-branch',
                    iconColor: 'text-bolt-elements-icon-info',
                  },
                  {
                    label: getTranslation('contributors', language),
                  ...
                  {
                    label: getTranslation('pullRequests', language),
            ...
            {stats.organizations && stats.organizations.length > 0 && (
              <div>
                <h5 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('organizations', language)}</h5>
            ...
            <div className="pt-2 border-t border-bolt-elements-borderColor">
              <span className="text-xs text-bolt-elements-textSecondary">
                {getTranslation('lastUpdated', language)} {stats.lastUpdated ? new Date(stats.lastUpdated).toLocaleString() : getTranslation('never', language)}
              </span>
            </div>
          </div>
        </CollapsibleContent>
