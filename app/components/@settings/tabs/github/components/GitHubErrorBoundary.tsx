    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center space-y-4 bg-bolt-elements-background-depth-1 border border-bolt-elements-borderColor rounded-lg">
          ...
          <div>
            <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('githubIntegrationError', 'zh')}</h3>
            <p className="text-sm text-bolt-elements-textSecondary mb-4 max-w-md">
              {getTranslation('githubIntegrationErrorDesc', 'zh')}
            </p>

            {this.state.error && (
              <details className="text-xs text-bolt-elements-textTertiary mb-4">
                <summary className="cursor-pointer hover:text-bolt-elements-textSecondary">{getTranslation('showErrorDetails', 'zh')}</summary>
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={this.handleRetry}>
              {getTranslation('tryAgain', 'zh')}
            </Button>
            <Button variant="outline" size="sm" onClick={() => window.location.reload()}>
              {getTranslation('reloadPage', 'zh')}
            </Button>
          </div>
        </div>
      );
