...
export function RepositoryCard({
  ...
  const formatTimeAgo = () => {
    ...
    if (daysSinceUpdate === 1) {
      return getTranslation('oneDayAgo', language);
    }

    if (daysSinceUpdate < 7) {
      return getTranslation('daysAgo', language).replace('{days}', daysSinceUpdate.toString());
    }

    if (daysSinceUpdate < 30) {
      return getTranslation('weeksAgo', language).replace('{weeks}', Math.floor(daysSinceUpdate / 7).toString());
    }
  ...
  const getHealthTitle = () => {
    if (repository.archived) {
      return getTranslation('archived', language);
    }

    if (daysSinceUpdate < 7) {
      return getTranslation('veryActive', language);
    }

    if (daysSinceUpdate < 30 && repository.stargazers_count > 0) {
      return getTranslation('healthy', language);
    }

    return getTranslation('needsAttention', language);
  };
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            {repository.fork && (
              <span title={getTranslation('forkedRepository', language)}>
                <GitFork className="w-3 h-3 text-bolt-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title={getTranslation('archivedRepository', language)}>
          ...
          <div className="flex items-center gap-3 text-xs text-bolt-elements-textSecondary">
            ...
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title={getTranslation('openIssues', language)}>
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title={getTranslation('pullRequests', language)}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-bolt-elements-textSecondary">
            ...
            {showExtendedMetrics && repository.branches_count && (
              <span className="flex items-center gap-1" title={getTranslation('totalBranches', language)}>
                <GitFork className="w-3.5 h-3.5" />
                {repository.branches_count}
              </span>
            )}
            {showExtendedMetrics && repository.contributors_count && (
              <span className="flex items-center gap-1" title={getTranslation('contributors', language)}>
                <Users className="w-3.5 h-3.5" />
                {repository.contributors_count}
              </span>
            )}
            {repository.size && (
              <span className="flex items-center gap-1" title={getTranslation('size', language)}>
          ...
          <div className="flex items-center gap-2">
            ...
            {health && (
              <div
                ...
                title={`${getTranslation('healthScore', language)}: ${health.percentage}% (${health.score}/${health.maxScore})`}
              >
