...
export function GitHubConnectionRequired({ onConnect, className = '' }: GitHubConnectionRequiredProps) {
  ...
  return (
    <div className={classNames('flex flex-col items-center justify-center py-12 text-center', className)}>
      ...
      <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('githubConnectionRequired', language)}</h3>
      <p className="text-sm text-bolt-elements-textSecondary mb-6 max-w-md">
        {getTranslation('githubConnectionRequiredDesc', language)}
      </p>
      {onConnect && (
        <button
          onClick={onConnect}
        ...
        >
          ...
          {getTranslation('connectGitHub', language)}
        </button>
      )}
    </div>
  );
...
export function InformationState({
  ...
  const language = useStore(languageStore);
  const defaultActionLabel = actionLabel || getTranslation('gotIt', language);
  const sizeClasses = {
    sm: 'w-4 h-4',
    md: 'w-6 h-6',
    lg: 'w-8 h-8',
