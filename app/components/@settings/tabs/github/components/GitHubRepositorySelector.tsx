  ...
  if (!isConnected || !connection) {
    return (
      <div className="text-center p-8">
        <p className="text-bolt-elements-textSecondary mb-4">{getTranslation('pleaseConnectGitHub', language)}</p>
        <Button variant="outline" onClick={() => window.location.reload()}>
          {getTranslation('refreshConnection', language)}
        </Button>
      </div>
    );
  }
  ...
  if (isStatsLoading && !stats) {
    return (
      <div className="flex flex-col items-center justify-center p-8 space-y-4">
        ...
        <p className="text-sm text-bolt-elements-textSecondary">{getTranslation('loadingRepositories', language)}</p>
      </div>
    );
  }
  ...
  if (!repositories.length) {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-bolt-elements-textSecondary mb-4">{getTranslation('noRepositoriesFound', language)}</p>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-bolt-elements-textPrimary">{getTranslation('selectRepositoryToClone', language)}</h3>
          <p className="text-sm text-bolt-elements-textSecondary">
            {filteredRepositories.length} {getTranslation('of', language)} {repositories.length} {getTranslation('repositories', language)}
          </p>
        </div>
        <Button
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={getTranslation('searchRepositories', language)}
            value={searchQuery}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <option value="updated">{getTranslation('recentlyUpdated', language)}</option>
            <option value="stars">{getTranslation('mostStarred', language)}</option>
            <option value="name">{getTranslation('nameAZ', language)}</option>
            <option value="created">{getTranslation('recentlyCreated', language)}</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg bg-bolt-elements-background-depth-1 border border-bolt-elements-borderColor text-bolt-elements-textPrimary text-sm focus:outline-none focus:ring-1 focus:ring-bolt-elements-borderColorActive"
          >
            <option value="all">{getTranslation('allRepositories', language)}</option>
            <option value="own">{getTranslation('ownRepositories', language)}</option>
            <option value="forks">{getTranslation('forkedRepositories', language)}</option>
            <option value="archived">{getTranslation('archivedRepositories', language)}</option>
          </select>
        </div>
      </div>
      ...
      {/* Repository Grid */}
      {
        currentRepositories.length > 0 ? (
              ...
              <div className="flex items-center justify-between pt-4 border-t border-bolt-elements-borderColor">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-bolt-elements-textSecondary px-3">
                    {currentPage} {getTranslation('of', language)} {totalPages}
                  </span>
                  <Button
          ...
          <div className="text-center py-8">
            <p className="text-bolt-elements-textSecondary">{getTranslation('noRepositoriesMatchingSearch', language)}</p>
          </div>
        )
      }
