    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-bolt-elements-textSecondary">{getTranslation('loadingConnection', language)}</span>
        </div>
      </div>
    );
  }
        ...
        {!isConnected && (
          <div className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 dark:bg-bolt-elements-background-depth-1 p-3 rounded-lg mb-4">
            <p className="flex items-center gap-1 mb-1">
              ...
              <span className="font-medium">{getTranslation('tipEnvVar', language)}</span> {getTranslation('tipGitHubEnvVar', language)}
            </p>
            <p>
              {getTranslation('tipFineGrainedEnv', language)}
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary mb-2">
                {getTranslation('tokenType', language)}
              </label>
              <select
                value={tokenType}
              ...
              >
                <option value="classic">{getTranslation('personalAccessTokenClassic', language)}</option>
                <option value="fine-grained">{getTranslation('fineGrainedToken', language)}</option>
              </select>
            </div>

            <div>
              <label className="block text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary mb-2">
                {tokenType === 'classic' ? getTranslation('personalAccessToken', language) : getTranslation('fineGrainedToken', language)}
              </label>
              <input
                type="password"
                value={token}
                ...
                disabled={isConnecting || isConnected}
                placeholder={tokenType === 'classic' ? getTranslation('enterGitHubToken', language) : getTranslation('enterGitHubFineGrainedToken', language)}
                className={classNames(
                  'w-full px-3 py-2 rounded-lg text-sm',
              ...
              <div className="mt-2 text-sm text-bolt-elements-textSecondary">
                ...
                <span>
                  {getTranslation('requiredScopes', language)}{' '}
                  {tokenType === 'classic' ? getTranslation('classicScopes', language) : getTranslation('fineGrainedScopes', language)}
                </span>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-bolt-elements-textSecondary flex items-center gap-1">
                    ...
                    {getTranslation('connectedToGitHub', language).replace('{username}', connection?.user?.login || '')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Button
