    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-bolt-elements-textPrimary">{getTranslation('githubCacheManagement', language)}</h3>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-bolt-elements-textSecondary">{getTranslation('totalSize', language)}</span>
            </div>
          ...
          <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-bolt-elements-textSecondary">{getTranslation('entries', language)}</span>
            </div>
          ...
          <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-bolt-elements-textSecondary">{getTranslation('oldest', language)}</span>
            </div>
          ...
          <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-bolt-elements-textSecondary">{getTranslation('status', language)}</span>
            </div>
            <p className="text-xs text-green-600 dark:text-green-400">
              {cacheStats.totalEntries > 0 ? getTranslation('active', language) : getTranslation('empty', language)}
            </p>
          </div>
        </div>
      )}
      ...
      {cacheEntries.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-bolt-elements-textSecondary">
            {getTranslation('cacheEntries', language)} ({cacheEntries.length})
          </h4>
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-bolt-elements-borderColor">
        ...
        >
          ...
          <span className="text-xs">{getTranslation('clearExpired', language)}</span>
        </Button>
        ...
        >
          ...
          <span className="text-xs">{getTranslation('compact', language)}</span>
        </Button>
        ...
        {cacheEntries.length > 0 && (
          ...
          >
            ...
            <span className="text-xs">{getTranslation('clearAll', language)}</span>
          </Button>
        )}
      </div>
      ...
      {lastClearTime && (
        <div className="flex items-center gap-2 p-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded text-xs text-green-700 dark:text-green-400">
          ...
          <span>{getTranslation('cacheClearedSuccessfully', language)} {new Date(lastClearTime).toLocaleTimeString()}</span>
        </div>
      )}
    </div>
  );
