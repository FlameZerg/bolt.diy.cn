          ...
          <div className="flex flex-col gap-1">
            ...
            {log.details && (
              <>
                ...
                >
                  {localExpanded ? getTranslation('hideDetails', useStore(languageStore)) : getTranslation('showDetails', useStore(languageStore))}
                </button>
...
export function EventLogsTab() {
  ...
  const logLevelOptions: SelectOption[] = useMemo(() => [
    {
      value: 'all',
      label: getTranslation('allTypes', language),
      icon: 'i-ph:funnel',
      color: '#9333ea',
    },
    {
      value: 'provider',
      label: getTranslation('llm', language),
      icon: 'i-ph:robot',
      color: '#10b981',
    },
    {
      value: 'api',
      label: getTranslation('api', language),
      icon: 'i-ph:cloud',
      color: '#3b82f6',
    },
    {
      value: 'error',
      label: getTranslation('error', language),
      icon: 'i-ph:warning-circle',
      color: '#ef4444',
    },
    {
      value: 'warning',
      label: getTranslation('warning', language),
      icon: 'i-ph:warning',
      color: '#f59e0b',
    },
    {
      value: 'info',
      label: getTranslation('info', language),
      icon: 'i-ph:info',
      color: '#3b82f6',
    },
    {
      value: 'debug',
      label: getTranslation('debug', language),
      icon: 'i-ph:bug',
      color: '#6b7280',
    },
  ], [language]);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      logStore.logSuccess(getTranslation('logsRefreshed', languageStore.get()), {
        type: 'refresh',
  ...
  const exportAsJSON = () => {
    try {
      ...
      toast.success(getTranslation('logsExported', languageStore.get()));
    } catch (error) {
      console.error('Failed to export JSON:', error);
      toast.error(getTranslation('failedToExportLogs', languageStore.get()));
    }
  };
  ...
  const exportFormats: ExportFormat[] = [
    {
      id: 'json',
      label: getTranslation('exportJson', languageStore.get()),
      icon: 'i-ph:file-js',
      handler: exportAsJSON,
    },
    {
      id: 'csv',
      label: getTranslation('exportCsv', languageStore.get()),
      icon: 'i-ph:file-csv',
      handler: exportAsCSV,
    },
    {
      id: 'pdf',
      label: getTranslation('exportPdf', languageStore.get()),
      icon: 'i-ph:file-pdf',
      handler: exportAsPDF,
    },
    {
      id: 'txt',
      label: getTranslation('exportText', languageStore.get()),
      icon: 'i-ph:file-text',
        ...
        <Dialog showCloseButton>
          <div className="p-6">
            <DialogTitle className="flex items-center gap-2">
              ...
              {getTranslation('exportEventLogs', language)}
            </DialogTitle>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-bolt-elements-textSecondary mt-0.5">
                      {format.id === 'json' && getTranslation('exportAsJsonDesc', language)}
                      {format.id === 'csv' && getTranslation('exportAsCsvDesc', language)}
                      {format.id === 'pdf' && getTranslation('exportAsPdfDesc', language)}
                      {format.id === 'txt' && getTranslation('exportAsTextDesc', language)}
                    </div>
                  </div>
                </button>
              ))}
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
          <DropdownMenu.Trigger asChild>
            ...
            >
              ...
              />
              {selectedLevelOption?.label || getTranslation('allTypes', language)}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('showTimestamps', language)}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-purple-500"
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('time24h', language)}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-purple-500"
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('autoExpand', language)}</span>
          </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          <input
            type="text"
            placeholder={getTranslation('searchLogs', language)}
            value={searchQuery}
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white">{getTranslation('noLogsFound', language)}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('tryAdjustingSearch', language)}</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => (
