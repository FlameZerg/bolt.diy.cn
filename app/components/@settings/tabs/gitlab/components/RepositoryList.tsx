    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-bolt-elements-textPrimary">
          {getTranslation('repositories', language)} ({filteredRepositories.length})
        </h4>
        {onRefresh && (
          <Button
            onClick={onRefresh}
      ...
      <div className="space-y-4">
        {filteredRepositories.length === 0 ? (
          <div className="text-center py-8 text-bolt-elements-textSecondary">
            {searchQuery ? getTranslation('noRepositoriesMatchingSearch', language) : getTranslation('noRepositoriesAvailable', language)}
          </div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-4 border-t border-bolt-elements-borderColor">
                <div className="text-sm text-bolt-elements-textSecondary">
                  {getTranslation('showingResults', language).replace('{start}', Math.min(startIndex + 1, filteredRepositories.length).toString()).replace('{end}', Math.min(endIndex, filteredRepositories.length).toString()).replace('{total}', filteredRepositories.length.toString())}
                </div>
