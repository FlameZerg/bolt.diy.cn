  ...
  if (!isConnected || !connection) {
    return (
      <div className="text-center p-8">
        <p className="text-bolt-elements-textSecondary mb-4">{getTranslation('gitlabConnectionRequiredDesc', language)}</p>
    ...
    >
      ...
      {error && repositories.length > 0 && (
        <div className="p-3 rounded-lg bg-yellow-50 border border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-700">
          <p className="text-sm text-yellow-800 dark:text-yellow-200">{getTranslation('warningCachedData', language).replace('{error}', error)}</p>
        </div>
      )}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t border-bolt-elements-borderColor">
              <div className="text-sm text-bolt-elements-textSecondary">
                {getTranslation('showingRepositories', language)
                  .replace('{current}', Math.min(startIndex + 1, filteredRepositories.length).toString())
                  .replace('{end}', Math.min(startIndex + REPOS_PER_PAGE, filteredRepositories.length).toString())
                  .replace('{total}', filteredRepositories.length.toString())}
              </div>
              <div className="flex items-center gap-2">
                <Button
