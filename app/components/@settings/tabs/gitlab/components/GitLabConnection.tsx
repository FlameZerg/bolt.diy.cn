      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-medium text-bolt-elements-textPrimary">{getTranslation('gitlabConnection', language)}</h3>
          </div>
        </div>
        ...
        {!isConnected && (
          <div className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 p-3 rounded-lg mb-4">
            <p className="flex items-center gap-1 mb-1">
              ...
              <span className="font-medium">{getTranslation('tipEnvVar', language)}</span> {getTranslation('tipGitLabEnvVar', language)}
            </p>
            <p>
              {getTranslation('tipGitLabSelfHosted', language)}
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleConnect}>
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm text-bolt-elements-textSecondary mb-2">{getTranslation('gitlabUrl', language)}</label>
              <input
                type="text"
                value={gitlabUrl}
            ...
            <div>
              ...
              <input
                ...
                disabled={isConnecting || isConnected}
                placeholder={getTranslation('enterGitLabToken', language)}
                className={classNames(
                  'w-full px-3 py-2 rounded-lg text-sm',
                  'bg-bolt-elements-background-depth-1',
                  'border border-bolt-elements-borderColor',
              ...
              <div className="mt-2 text-sm text-bolt-elements-textSecondary">
                ...
                <span>{getTranslation('gitlabTokenScopes', language)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            {!isConnected ? (
              <>
                ...
                >
                  {getTranslation('testValues', language)}
                </button>
              </>
            ) : (
              <>
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-bolt-elements-textSecondary flex items-center gap-1">
                      ...
                      {getTranslation('connectedToGitLab', language)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
