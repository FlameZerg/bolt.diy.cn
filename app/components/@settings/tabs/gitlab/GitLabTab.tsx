  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (isValid) {
        setConnectionTest({
          status: 'success',
          message: getTranslation('connectedSuccessfully', language).replace('{user}', connection.user.username),
          timestamp: Date.now(),
        });
      } else {
        setConnectionTest({
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <GitLabLogo />
          <h2 className="text-lg font-medium text-bolt-elements-textPrimary">{getTranslation('gitlabIntegration', language)}</h2>
        </div>
        <div className="flex items-center justify-center p-4">
          <div className="flex items-center gap-2">
            <div className="i-ph:spinner-gap-bold animate-spin w-4 h-4" />
            <span className="text-bolt-elements-textSecondary">{getTranslation('loading', language)}</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-bolt-elements-textSecondary">
          {getTranslation('gitlabDescription', language)}
        </p>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary">
            {getTranslation('gitlabIntegration', language)}
          </h2>
        </div>
        <div className="flex items-center gap-2">
      ...
      {connection?.stats && (
        <motion.div
          ...
          className="border-t border-bolt-elements-borderColor pt-6"
        >
          <h3 className="text-base font-medium text-bolt-elements-textPrimary mb-4">{getTranslation('gitlabStats', language)}</h3>
          <StatsDisplay
