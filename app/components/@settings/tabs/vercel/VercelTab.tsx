...
export default function VercelTab() {
  ...
  const projectActions: ProjectAction[] = useMemo(
    () => [
      {
        name: getTranslation('redeploy', language),
        ...
        action: async (projectId: string) => {
          try {
            ...
            if (!response.ok) {
              throw new Error(getTranslation('failedToRedeploy', language));
            }

            toast.success(getTranslation('projectRedeployInitiated', language));
            await fetchVercelStats(connection.token);
          } catch (err: unknown) {
            const error = err instanceof Error ? err.message : 'Unknown error';
            toast.error(getTranslation('failedToRedeploy', language).replace('{error}', error));
          }
        },
      },
      {
      ...
      {
        name: getTranslation('viewDeployments', language),
        icon: 'i-ph:rocket',
        action: async (projectId: string) => {
      ...
      {
        name: getTranslation('viewAnalytics', language),
        icon: 'i-ph:chart-bar',
        action: async (projectId: string) => {
      ...
      {
        name: getTranslation('viewDomains', language),
        icon: 'i-ph:globe',
        action: async (projectId: string) => {
      ...
      {
        name: getTranslation('deleteProject', language),
        icon: 'i-ph:trash',
        action: async (projectId: string) => {
          try {
            ...
            if (!response.ok) {
              throw new Error(getTranslation('failedToDeleteProject', language));
            }

            toast.success(getTranslation('projectDeleted', language));
            await fetchVercelStats(connection.token);
          } catch (err: unknown) {
            const error = err instanceof Error ? err.message : 'Unknown error';
            toast.error(getTranslation('failedToDeleteProject', language).replace('{error}', error));
          }
        },
  ...
  const handleConnect = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!token.trim()) {
        throw new Error(getTranslation('tokenRequired', language));
      }
      ...
      if (!testResponse.ok) {
        if (testResponse.status === 401) {
          throw new Error(getTranslation('invalidVercelToken', language));
        }

        throw new Error(getTranslation('vercelApiError', language).replace('{status}', testResponse.status.toString()));
      }
      ...
      toast.success(getTranslation('connectedToVercel', language));
    } catch (error) {
      console.error('Auth error:', error);
      logStore.logError('Failed to authenticate with Vercel', { error });

      const errorMessage = error instanceof Error ? error.message : getTranslation('failedToConnectVercel', language);
  ...
  const handleDisconnect = () => {
    ...
    updateVercelConnection({ user: null, token: '' });
    toast.success(getTranslation('disconnectedFromVercel', language));
  };
    ...
    if (fetchingStats) {
      return (
        <div className="flex items-center gap-2 text-sm text-bolt-elements-textSecondary">
          ...
          {getTranslation('fetchingVercelProjects', language)}
        </div>
      );
    }
          ...
          <div className="space-y-4 mt-4">
            ...
            {connection.stats?.projects?.length ? (
              <div className="mb-6 p-4 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{getTranslation('vercelOverview', language)}</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('deployedProjects', language)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {new Set(connection.stats.projects.map((p) => p.framework).filter(Boolean)).size}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('frameworksUsed', language)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                      {connection.stats.projects.filter((p) => p.latestDeployments?.[0]?.state === 'READY').length}
                    </div>
                    <div className="text-xs text-bolt-elements-textSecondary">{getTranslation('activeDeployments', language)}</div>
                  </div>
                </div>
              </div>
            ) : null}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg border border-bolt-elements-borderColor">
                    <h6 className="text-xs font-medium text-bolt-elements-textPrimary flex items-center gap-2 mb-2">
                      ...
                      {getTranslation('deploymentHealth', language)}
                    </h6>
                    <div className="space-y-1">
                      {(() => {
                        ...
                        return [
                          ...
                          { label: getTranslation('active', language) || 'Active', value: readyDeployments },
                          { label: getTranslation('error', language), value: errorDeployments },
                        ];
                      })().map((item, idx) => (
                  ...
                  <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg border border-bolt-elements-borderColor">
                    <h6 className="text-xs font-medium text-bolt-elements-textPrimary flex items-center gap-2 mb-2">
                      ...
                      {getTranslation('frameworkDistribution', language)}
                    </h6>
                    <div className="space-y-1">
                      {(() => {
                  ...
                  <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg border border-bolt-elements-borderColor">
                    <h6 className="text-xs font-medium text-bolt-elements-textPrimary flex items-center gap-2 mb-2">
                      ...
                      {getTranslation('activitySummary', language)}
                    </h6>
                    <div className="space-y-1">
                      {(() => {
                        const now = Date.now();
                        ...
                        return [
                          { label: getTranslation('recentDeploys', language), value: recentDeployments },
                          { label: getTranslation('totalDomains', language), value: totalDomains },
                          { label: getTranslation('avgDomainsPerProject', language), value: avgDomainsPerProject },
                        ];
                      })().map((item, idx) => (
                        <div key={idx} className="flex justify-between text-xs">
                          <span className="text-bolt-elements-textSecondary">{item.label}:</span>
            ...
            {connection.stats?.projects?.length ? (
              <div className="mb-6">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('projectHealthOverview', language)}</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {(() => {
                    ...
                    return [
                      ...
                      {
                        label: getTranslation('customDomain', language),
                      ...
                      {
                        label: getTranslation('building', language),
                      ...
                      {
                        label: getTranslation('issues', language),
            ...
            {connection.stats?.projects?.length ? (
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mt-3 pt-3 border-t border-bolt-elements-borderColor">
                          <div className="text-center">
                            ...
                            <div className="text-xs text-bolt-elements-textSecondary flex items-center justify-center gap-1">
                              ...
                              {getTranslation('viewDeployments', language)}
                            </div>
                          </div>
                          <div className="text-center">
                            <div className="text-sm font-semibold text-bolt-elements-textPrimary">
                            ...
                            <div className="text-xs text-bolt-elements-textSecondary flex items-center justify-center gap-1">
                              ...
                              {getTranslation('viewDomains', language)}
                            </div>
                          </div>
                          <div className="text-center">
                            <div className="text-sm font-semibold text-bolt-elements-textPrimary">
                            ...
                            <div className="text-xs text-bolt-elements-textSecondary flex items-center justify-center gap-1">
                              ...
                              {getTranslation('team', language) || 'Team'}
                            </div>
                          </div>
                          <div className="text-center">
                            <div className="text-sm font-semibold text-bolt-elements-textPrimary">
                            ...
                            <div className="text-xs text-bolt-elements-textSecondary flex items-center justify-center gap-1">
                              ...
                              {getTranslation('bandwidth', language) || 'Bandwidth'}
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          {getTranslation('view', language) || 'View'}
                        </Button>
                      </div>
                    </div>
              ...
              <div className="text-sm text-bolt-elements-textSecondary flex items-center gap-2 p-4">
                ...
                {getTranslation('noVercelProjects', language)}
              </div>
            )}
          </div>
        </CollapsibleContent>
  ...
  return (
    <div className="space-y-6">
      <ServiceHeader
        icon={VercelLogo}
        title={getTranslation('vercelIntegration', language)}
        description={getTranslation('vercelDescription', language)}
            ...
            <div className="space-y-4">
              <div className="text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 dark:bg-bolt-elements-background-depth-1 p-3 rounded-lg mb-4">
                <p className="flex items-center gap-1 mb-1">
                  ...
                  <span className="font-medium">{getTranslation('tip', language)}:</span> {getTranslation('vercelEnvTip', language).replace('{envVar}', 'VITE_VERCEL_ACCESS_TOKEN')}
                </p>
              </div>

              <div>
                <label className="block text-sm text-bolt-elements-textSecondary mb-2">{getTranslation('personalAccessToken', language)}</label>
                <input
                  type="password"
                  value={connection.token}
                  onChange={(e) => updateVercelConnection({ ...connection, token: e.target.value })}
                  disabled={connecting}
                  placeholder={getTranslation('enterVercelToken', language)}
                  className={classNames(
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-bolt-elements-textSecondary flex items-center gap-1">
                    ...
                    {getTranslation('connectedToVercel', language)}
                  </span>
                </div>
              </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-bolt-elements-textSecondary">
                      <span className="flex items-center gap-1">
                        ...
                        {connection.stats?.totalProjects || 0} {getTranslation('projects', language)}
                      </span>
                      <span className="flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-3 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="i-ph:buildings w-4 h-4 text-bolt-elements-item-contentAccent" />
                      <span className="text-xs font-medium text-bolt-elements-textPrimary">{getTranslation('projects', language)}</span>
                    </div>
                    <div className="text-sm text-bolt-elements-textSecondary">
                      ...
                      <div>{getTranslation('total', language)}: {connection.stats?.totalProjects || 0}</div>
                    </div>
                  </div>
                  <div className="p-3 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="i-ph:globe w-4 h-4 text-bolt-elements-item-contentAccent" />
                      <span className="text-xs font-medium text-bolt-elements-textPrimary">{getTranslation('domains', language)}</span>
                    </div>
                    <div className="text-sm text-bolt-elements-textSecondary">
                      {/* Domain usage would be fetched from API */}
                      <div>{getTranslation('custom', language)}: --</div>
                      <div>Vercel: --</div>
                    </div>
                  </div>
                  <div className="p-3 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="i-ph:activity w-4 h-4 text-bolt-elements-item-contentAccent" />
                      <span className="text-xs font-medium text-bolt-elements-textPrimary">{getTranslation('usage', language)}</span>
                    </div>
                    <div className="text-sm text-bolt-elements-textSecondary">
                      {/* Usage metrics would be fetched from API */}
                      <div>{getTranslation('bandwidth', language)}: --</div>
                      <div>{getTranslation('requests', language)}: --</div>
                    </div>
                  </div>
                </div>
              </div>
