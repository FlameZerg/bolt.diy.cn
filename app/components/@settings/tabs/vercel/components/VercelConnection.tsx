...
export default function VercelConnection() {
  ...
  useEffect(() => {
    ...
    const initializeConnection = async () => {
      ...
      if (!connection.user && connection.token && import.meta.env?.VITE_VERCEL_ACCESS_TOKEN) {
        ...
        if (result.success) {
          toast.success(getTranslation('connectedToVercelAuto', language));
        } else {
  ...
  const handleConnect = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(getTranslation('failedToConnectVercel', language));
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            />
            <h3 className="text-base font-medium text-bolt-elements-textPrimary">{getTranslation('vercelIntegration', language)}</h3>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={connecting}
                placeholder={getTranslation('enterAccessToken', language)}
              ...
              <div className="mt-2 text-sm text-bolt-elements-textSecondary">
                ...
                <div className="mt-2 text-xs text-bolt-elements-textSecondary bg-bolt-elements-background-depth-1 p-2 rounded">
                  <p className="flex items-center gap-1">
                    ...
                    <span className="font-medium">{getTranslation('tip', language)}:</span> {getTranslation('envVariableTip', language).replace('GITHUB', 'VITE_VERCEL_ACCESS_TOKEN')}
                  </p>
                </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={async () => {
                  ...
                  if (result.success) {
                    toast.success(getTranslation('manualConnectSuccess', language) || 'Manual auto-connect successful');
                  } else {
                    toast.error(getTranslation('manualConnectFailed', language).replace('{error}', result.error));
                  }
                }}
            ...
            <div className="flex items-center gap-4 p-4 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg">
              ...
              <div>
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary">
                  {connection.user?.username || connection.user?.user?.username || getTranslation('vercelUser', language)}
                </h4>
                <p className="text-sm text-bolt-elements-textSecondary">
                  {connection.user?.email || connection.user?.user?.email || getTranslation('noEmailAvailable', language)}
                </p>
              </div>
            </div>
            ...
            {fetchingStats ? (
              ...
              <div>
                ...
                >
                  ...
                  {getTranslation('yourProjects', language)} ({connection.stats?.totalProjects || 0})
                  <div
                    className={classNames(
                ...
                {isProjectsExpanded && connection.stats?.projects?.length ? (
                  ...
                  <div className="text-sm text-bolt-elements-textSecondary flex items-center gap-2">
                    ...
                    {getTranslation('noProjectsFound', language)}
                  </div>
                ) : null}
              </div>
            )}
