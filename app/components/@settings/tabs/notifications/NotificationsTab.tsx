...
const NotificationsTab = () => {
  ...
  const dateLocale = language === 'zh' ? zhCN : undefined;
      ...
      return (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1 text-xs text-gray-500 dark:text-gray-500">
            <p>{getTranslation('version', language)}: {details.currentVersion}</p>
            <p>{getTranslation('latestBranch', language)}: {details.latestVersion}</p>
            <p>{getTranslation('branch', language)}: {details.branch}</p>
          </div>
          <button
          ...
          >
            ...
            {getTranslation('viewChanges', language)}
          </button>
        </div>
      );
    }
  ...
  const filterOptions: { id: FilterType; label: string; icon: string; color: string }[] = [
    { id: 'all', label: getTranslation('allNotifications', language), icon: 'i-ph:bell', color: '#9333ea' },
    { id: 'system', label: getTranslation('system', language), icon: 'i-ph:gear', color: '#6b7280' },
    { id: 'update', label: getTranslation('update', language), icon: 'i-ph:arrow-circle-up', color: '#9333ea' },
    { id: 'error', label: getTranslation('error', language), icon: 'i-ph:warning-circle', color: '#ef4444' },
    { id: 'warning', label: getTranslation('warning', language), icon: 'i-ph:warning', color: '#f59e0b' },
    { id: 'info', label: getTranslation('info', language), icon: 'i-ph:info', color: '#3b82f6' },
    { id: 'provider', label: getTranslation('provider', language), icon: 'i-ph:robot', color: '#10b981' },
    { id: 'network', label: getTranslation('network', language), icon: 'i-ph:wifi-high', color: '#6366f1' },
  ];
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            ...
            >
              ...
              />
              {filterOptions.find((opt) => opt.id === filter)?.label || getTranslation('filterNotifications', language)}
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white">{getTranslation('noNotifications', language)}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('allCaughtUp', language)}</p>
            </div>
          </motion.div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {getTranslation('category', language)}: {log.category}
