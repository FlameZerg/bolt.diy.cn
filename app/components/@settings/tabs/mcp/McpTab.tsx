...
export default function McpTab() {
  ...
  useEffect(() => {
    if (!isInitialized) {
      initialize().catch((err) => {
        ...
        toast.error(getTranslation('failedToLoadMcpConfig', language));
      });
    }
  }, [isInitialized]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(getTranslation('mcpConfigSaved', language));

      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to save configuration');
      toast.error(getTranslation('failedToSaveMcpConfig', language));
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <section aria-labelledby="server-status-heading">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-base font-medium text-bolt-elements-textPrimary">{getTranslation('mcpServersConfigured', language)}</h2>{' '}
          <button
          ...
          >
            ...
            {getTranslation('checkAvailability', language)}
          </button>
        </div>
        <McpServerList
      ...
      <section aria-labelledby="config-section-heading">
        <h2 className="text-base font-medium text-bolt-elements-textPrimary mb-3">{getTranslation('configuration', language)}</h2>

        <div className="space-y-4">
          <div>
            <label htmlFor="mcp-config" className="block text-sm text-bolt-elements-textSecondary mb-2">
              {getTranslation('configurationJson', language)}
            </label>
            <textarea
              id="mcp-config"
              value={mcpConfigText}
          ...
          <div>
            <label htmlFor="max-llm-steps" className="block text-sm text-bolt-elements-textSecondary mb-2">
              {getTranslation('maxLlmStepsLabel', language)}
            </label>
            <input
              id="max-llm-steps"
              type="number"
              placeholder={getTranslation('maxLlmStepsPlaceholder', language)}
              min="1"
              max="20"
              value={maxLLMSteps}
          ...
          <div className="mt-2 text-sm text-bolt-elements-textSecondary">
            {getTranslation('mcpConfigFormatHint', language)}
            <a
            ...
            >
              {getTranslation('viewExampleServers', language)}
      ...
      <div className="flex flex-wrap justify-between gap-3 mt-6">
        ...
        >
          {getTranslation('loadExample', language)}
        </button>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {isSaving ? getTranslation('saving', language) : getTranslation('saveConfiguration', language)}
          </button>
        </div>
      </div>
    </div>
