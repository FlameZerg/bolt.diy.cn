...
const CloudProvidersTab = () => {
  ...
  const handleToggleCategory = useCallback(
    (enabled: boolean) => {
      ...
      toast.success(enabled ? getTranslation('allCloudProvidersEnabled', language) : getTranslation('allCloudProvidersDisabled', language));
    },
    [filteredProviders, settings, language],
  );
  ...
  const handleToggleProvider = useCallback(
    (provider: IProviderConfig, enabled: boolean) => {
      ...
      if (enabled) {
        ...
        toast.success(getTranslation('providerEnabled', language).replace('{provider}', provider.name));
      } else {
        logStore.logProvider(`Provider ${provider.name} disabled`, { provider: provider.name });
        toast.success(getTranslation('providerDisabled', language).replace('{provider}', provider.name));
      }
    },
    [settings, language],
  );
  ...
  const handleUpdateBaseUrl = useCallback(
    (provider: IProviderConfig, baseUrl: string) => {
      ...
      toast.success(getTranslation('baseUrlUpdated', language).replace('{provider}', provider.name));
      ...
      >
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-bolt-elements-textPrimary">{getTranslation('cloudProviders', language)}</h4>
              <p className="text-sm text-bolt-elements-textSecondary">{getTranslation('connectToCloudAI', language)}</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-sm text-bolt-elements-textSecondary">{getTranslation('enableAllCloud', language)}</span>
              ...
              <div className="absolute top-0 right-0 p-2 flex gap-1">
                {URL_CONFIGURABLE_PROVIDERS.includes(provider.name) && (
                  <motion.span
                    ...
                    whileTap={{ scale: 0.95 }}
                  >
                    {getTranslation('configurable', language)}
                  </motion.span>
                )}
              </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-bolt-elements-textSecondary mt-0.5">
                        {PROVIDER_DESCRIPTIONS[provider.name as keyof typeof PROVIDER_DESCRIPTIONS]
                          ? getTranslation(PROVIDER_DESCRIPTIONS[provider.name as keyof typeof PROVIDER_DESCRIPTIONS] as any, language)
                          : (URL_CONFIGURABLE_PROVIDERS.includes(provider.name)
                            ? getTranslation('configureCustomEndpoint', language)
                            : getTranslation('standardAiProvider', language))}
                      </p>
                    </div>
                    <Switch
                      ...
                      <div className="flex items-center gap-2 mt-4">
                        {editingProvider === provider.name ? (
                          <input
                            ...
                            placeholder={getTranslation('enterProviderBaseUrl', language).replace('{provider}', provider.name)}
                            className={classNames(
                          ...
                          >
                            <div className="flex items-center gap-2 text-bolt-elements-textSecondary">
                              ...
                              <span className="group-hover/url:text-purple-500 transition-colors">
                                {provider.settings.baseUrl || getTranslation('clickToSetBaseUrl', language)}
                              </span>
                            </div>
                          </div>
                        )}
                      ...
                      {providerBaseUrlEnvKeys[provider.name]?.baseUrlKey && (
                        <div className="mt-2 text-xs text-green-500">
                          <div className="flex items-center gap-1">
                            ...
                            <span>{getTranslation('envUrlSet', language)}</span>
                          </div>
                        </div>
                      )}
                    </motion.div>
