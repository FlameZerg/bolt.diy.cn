...
export default function LocalProvidersTab() {
  ...
  const handleToggleCategory = useCallback(
    async (enabled: boolean) => {
      ...
      toast(enabled ? getTranslation('allLocalProvidersEnabled', language) : getTranslation('allLocalProvidersDisabled', language));
    },
    [filteredProviders, updateProviderSettings, toast, language],
  );
  ...
  const handleToggleProvider = useCallback(
    (provider: IProviderConfig, enabled: boolean) => {
      ...
      if (enabled) {
        toast(getTranslation('providerEnabled', language).replace('{provider}', provider.name));
      } else {
        toast(getTranslation('providerDisabled', language).replace('{provider}', provider.name));
      }
    },
    [updateProviderSettings, toast, language],
  );
  ...
  const handleUpdateBaseUrl = useCallback(
    (provider: IProviderConfig, newBaseUrl: string) => {
      ...
      toast(getTranslation('baseUrlUpdated', language).replace('{provider}', provider.name));
    },
    [updateProviderSettings, toast, language],
  );
  ...
  const handleUpdateOllamaModel = async (modelName: string) => {
    try {
      ...
      toast(getTranslation('successfullyUpdated', language).replace('{name}', modelName));
    } catch {
      setOllamaModels((prev) =>
        prev.map((m) => (m.name === modelName ? { ...m, status: 'error', progress: undefined } : m)),
      );
      toast(getTranslation('failedToUpdate', language).replace('{name}', modelName), { type: 'error' });
    }
  };

  const handleDeleteOllamaModel = async (modelName: string) => {
    if (!window.confirm(getTranslation('confirmDeleteModel', language).replace('{name}', modelName))) {
      return;
    }
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-bolt-elements-textPrimary">{getTranslation('localAIProviders', language)}</h2>
              <p className="text-sm text-bolt-elements-textSecondary">{getTranslation('configureLocalModels', language)}</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-bolt-elements-textSecondary">{getTranslation('enableAll', language)}</span>
              <Switch
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {getTranslation('setupGuide', language)}
              </Button>
              <Button
                variant="outline"
                size="sm"
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-semibold text-bolt-elements-textPrimary">{getTranslation('installedModels', language)}</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <CardContent className="space-y-4">
                    {isLoadingModels ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('noModelsInstalled', language)}</h3>
                        <p className="text-sm text-bolt-elements-textSecondary mb-4">
                          {(() => {
                            const translation = getTranslation('visitOllamaLibrary', language);
                            const parts = translation.split('{url}');
                            return (
                              <>
                                {parts[0]}
                                <a
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="flex-1 text-center font-medium">{getTranslation('browseModels', language)}</span>
                          </a>
                        </Button>
                      </div>
                    ) : (
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-semibold text-bolt-elements-textPrimary">{getTranslation('availableModels', language)}</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <CardContent className="space-y-4">
                    {isLoadingLMStudioModels ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('noModelsAvailable', language)}</h3>
                        <p className="text-sm text-bolt-elements-textSecondary mb-4">
                          {getTranslation('lmStudioRunningCheck', language)}
                        </p>
                        <Button
                          variant="outline"
                          size="sm"
                        ...
                        >
                          ...
                          >
                            <ExternalLink className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300 flex-shrink-0" />
                            <span className="flex-1 text-center font-medium">{getTranslation('getLMStudio', language)}</span>
                          </a>
                        </Button>
                      </div>
                    ) : (
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 text-xs text-bolt-elements-textSecondary">
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span>{getTranslation('ownedBy', language)}: {model.owned_by}</span>
                                  </div>
                                  {model.created && (
                                    <div className="flex items-center gap-1">
                                      <Activity className="w-3 h-3" />
                                      <span>{getTranslation('createdOn', language)}: {new Date(model.created * 1000).toLocaleDateString()}</span>
                                    </div>
                                  )}
                                </div>
                              </div>
        ...
        {filteredProviders.length === 0 && (
          <Card className="bg-bolt-elements-background-depth-2">
            <CardContent className="p-8 text-center">
              ...
              <Server className="w-16 h-16 mx-auto text-bolt-elements-textTertiary mb-4" />
              <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('noLocalProvidersAvailable', language)}</h3>
              <p className="text-sm text-bolt-elements-textSecondary">
                {getTranslation('localProvidersInfo', language)}
              </p>
            </CardContent>
          </Card>
        )}
