  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-xl font-semibold text-bolt-elements-textPrimary">{getTranslation('providerStatus', language)}</h2>
          <p className="text-sm text-bolt-elements-textSecondary">{getTranslation('monitorLocalProviders', language)}</p>
        </div>
      </div>
      ...
      {healthStatuses.length === 0 ? (
        <Card className="bg-bolt-elements-background-depth-2">
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-bolt-elements-textPrimary mb-2">{getTranslation('noEndpointsConfigured', language)}</h3>
            <p className="text-sm text-bolt-elements-textSecondary">
              {getTranslation('configureLocalProviders', language)}
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div className="text-center">
                    <div className="text-bolt-elements-textSecondary">{getTranslation('models', language)}</div>
                    <div className="text-lg font-semibold text-bolt-elements-textPrimary">
                      {status.availableModels?.length || 0}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-bolt-elements-textSecondary">{getTranslation('version', language)}</div>
                    <div className="text-lg font-semibold text-bolt-elements-textPrimary">
                      {status.version || getTranslation('unknown', language)}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-bolt-elements-textSecondary">{getTranslation('lastCheck', language)}</div>
                    <div className="text-lg font-semibold text-bolt-elements-textPrimary">
                      {status.lastChecked ? new Date(status.lastChecked).toLocaleTimeString() : getTranslation('never', language)}
                    </div>
                  </div>
                </div>
              </CardContent>
