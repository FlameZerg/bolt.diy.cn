...
export default function ProfileTab() {
  ...
  const debouncedUpdate = useCallback(
    debounce((field: 'username' | 'bio', value: string) => {
      ...
      toast.success(`${getTranslation(field, language)} ${getTranslation('updated', language)}`);
    }, 1000),
    [],
  );
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = () => {
        ...
        toast.success(getTranslation('profilePictureUpdated', language));
      };

      reader.onerror = () => {
        console.error('Error reading file:', reader.error);
        setIsUploading(false);
        toast.error(getTranslation('profilePictureUpdateFailed', language));
      };
        ...
        <div>
          ...
          <div className="flex items-start gap-6 mb-8">
            ...
            <div className="flex-1 pt-1">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{getTranslation('uploadProfilePicture', language)}</p>
            </div>
          </div>

          {/* Username Input */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">{getTranslation('username', language)}</label>
          ...
          {/* Bio Input */}
          <div className="mb-8">
            <label className="block text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">{getTranslation('bio', language)}</label>
            <div className="relative group">
              ...
              <textarea
                ...
                placeholder={getTranslation('tellUsAboutYourself', language)}
              />
            </div>
          </div>
        </div>
