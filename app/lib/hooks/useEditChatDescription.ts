...
export function useEditChatDescription({
  ...
  const isValidDescription = useCallback((desc: string): boolean => {
    ...
    if (!lengthValid) {
      toast.error(getTranslation('descLengthErr', language));
      return false;
    }

    if (!characterValid) {
      toast.error(getTranslation('descCharErr', language));
      return false;
    }
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        ...
        if (!chatId) {
          toast.error(getTranslation('chatIdUnavailable', language));
          return;
        }
        ...
        toast.success(getTranslation('descUpdated', language));
      } catch (error) {
        toast.error(getTranslation('descUpdateFailed', language) + ': ' + (error as Error).message);
      }
