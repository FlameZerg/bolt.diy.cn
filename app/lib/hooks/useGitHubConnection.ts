...
export function useGitHubConnection(): UseGitHubConnectionReturn {
  ...
  const loadSavedConnection = useCallback(async () => {
    ...
    try {
      ...
      setError(getTranslation('failedToLoadSavedConnection', languageStore.get()));
      setIsLoading(false);
  ...
  const connect = useCallback(async (token: string, tokenType: 'classic' | 'fine-grained') => {
    ...
    if (!token.trim()) {
      ...
      setError(getTranslation('tokenRequired', languageStore.get()));
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(getTranslation('authFailed', languageStore.get()).replace('{status}', String(response.status)).replace('{statusText}', response.statusText));
      }
      ...
      toast.success(getTranslation('connectedToGitHub', languageStore.get()).replace('{username}', userData.login));
    } catch (error) {
      console.error('Failed to connect to GitHub:', error);

      const errorMessage = error instanceof Error ? error.message : getTranslation('failedToConnect', languageStore.get()).replace('{error}', 'GitHub');

      setError(errorMessage);
      toast.error(getTranslation('failedToConnect', languageStore.get()).replace('{error}', errorMessage));
      throw error;
    } finally {
  ...
  const disconnect = useCallback(() => {
    ...
    setError(null);
    toast.success(getTranslation('disconnectedFromGitHub', languageStore.get()));
  }, []);

  const refreshConnection = useCallback(async () => {
    if (!connection?.token) {
      throw new Error(getTranslation('noConnectionToRefresh', languageStore.get()));
    }
    ...
    try {
      ...
      setError(getTranslation('failedToRefreshConnection', languageStore.get()));
      throw error;
    } finally {
      setIsLoading(false);
    }
