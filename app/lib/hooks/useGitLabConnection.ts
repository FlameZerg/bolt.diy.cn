...
export function useGitLabConnection(): UseGitLabConnectionReturn {
  ...
  const connect = useCallback(async (token: string, gitlabUrl = 'https://gitlab.com') => {
    ...
    try {
      ...
      toast.success(getTranslation('connectedToGitLab', languageStore.get()));
    } catch (error) {
      console.error('Failed to connect to GitLab:', error);

      const errorMessage = error instanceof Error ? error.message : getTranslation('failedToConnect', languageStore.get()).replace('{error}', 'GitLab');
  ...
  const disconnect = useCallback(() => {
    ...
    setError(null);
    toast.success(getTranslation('disconnectedFromGitLab', languageStore.get()));
  }, []);
