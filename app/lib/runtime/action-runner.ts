...
export class ActionRunner {
  ...
  handleDeployAction(
    ...
    const description =
      status === 'failed'
        ? `${stage === 'building' ? 'buildFailed' : 'deploymentFailed'}`
        : status === 'running'
          ? `${stage === 'building' ? 'buildingYourApplication' : 'deployingYourApplication'}`
          : status === 'complete'
            ? `${stage === 'building' ? 'buildCompletedSuccessfully' : 'deploymentCompletedSuccessfully'}`
            : `${stage === 'building' ? 'preparingToBuild' : 'preparingToDeploy'}`;
