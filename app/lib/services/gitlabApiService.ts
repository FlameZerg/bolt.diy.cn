...
export class GitLabApiService {
  ...
  async getUser(): Promise<GitLabUserResponse> {
    ...
    if (!response.ok) {
      let errorMessage = getTranslation('failedToFetchUser', languageStore.get()).replace('{status}', String(response.status));

      // Provide more specific error messages based on status code
      if (response.status === 401) {
        errorMessage = getTranslation('gitlab401Error', languageStore.get());
      } else if (response.status === 403) {
        errorMessage = getTranslation('gitlab403Error', languageStore.get());
      } else if (response.status === 404) {
        errorMessage = getTranslation('gitlab404Error', languageStore.get());
      } else if (response.status === 429) {
        errorMessage = getTranslation('gitlab429Error', languageStore.get());
      }
      ...
      try {
        ...
        if (errorData.message) {
          errorMessage += ' ' + getTranslation('errorDetails', languageStore.get()).replace('{details}', errorData.message);
        }
      } catch {
